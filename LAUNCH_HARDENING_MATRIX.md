...
| Private chat attachments | **IMPLEMENTED BUT DEPLOYMENT-DEPENDENT** | Authenticated chat upload uses strict data/type/signature/size validation, private non-static storage, opaque owner-bound records, owner-only retrieval/deletion, forced-download and no-cache headers, and stream-level attachment ownership validation. Malware scanning, binary moderation, object storage, encrypted-at-rest keys, retention policy, and multi-instance media durability remain deployment-dependent. |
