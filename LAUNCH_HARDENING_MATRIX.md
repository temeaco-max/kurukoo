...
| Private chat attachments | **IMPLEMENTED BUT DEPLOYMENT-DEPENDENT** | Authenticated chat upload uses strict data/type/signature/size validation, private non-static storage, opaque owner-bound records, owner-only retrieval/deletion, forced-download and no-cache headers, stream-level attachment ownership validation, cleanup when the final owner-scoped message/conversation reference is removed, cross-owner conversation-deletion protection, bounded owner-scoped metadata export without bytes or storage paths, and protected account-deletion cleanup of all owner chat history, metadata, and bytes. Malware scanning, binary moderation, object storage, encrypted-at-rest keys, retention policy, and multi-instance media durability remain deployment-dependent. |
