...
| Provider verification | **IMPLEMENTED BUT DEPLOYMENT-DEPENDENT** | `unverified`, `pending`, `verified`, `failed`, `expired`, and `suspended` states are evidence-gated and expiry-aware. |
...
| Advertising surfaces | **PARTIAL** | Surface and content-management boundaries exist; active advertiser inventory and commercial campaign evidence remain deployment-dependent. |
| Daily Picks and proactive prompts | **PARTIAL** | Existing non-blocking/dismissible primitives exist; production personalization, sponsorship, and re-engagement data require configured inputs. |
