...
async function main() {
  ...
  const briefRefs = (brief.items || []).map((i) => String(i.stableRef || ''));
  assert.ok(
    briefRefs.some((r) => r.includes(goal!.id) || r.includes(created!.id)),
  ...
  assert.ok(after.filter((r) => r.title === 'call the repair shop' && r.status === 'scheduled').length >= 1);
