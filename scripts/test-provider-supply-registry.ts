...
try {
  ...
  const imported = await fetch(`${baseUrl}/api/supply-registry/admin/entities`, { method: 'POST', headers: auth(adminToken), body: JSON.stringify({
    ...
    sourceType: 'public_website', sourceUrl: 'https://example.test/ikeja-screen-clinic', sourceRetrievedAt: '2026-08-13T09:00:00.000Z', sourceConfidence: 0.8,
  }) });
  assert.equal(imported.status, 201, 'admin can import a provenance-bearing public supply record');
  ...
  for (const status of ['verification_pending', 'verified']) {
    const transition = await fetch(`${baseUrl}/api/supply-registry/admin/entities/${encodeURIComponent(entityId)}/transition`, { method: 'POST', headers: auth(adminToken), body: JSON.stringify({ status }) });
    assert.equal(transition.status, 200, `registry can record ${status} lifecycle state without activating network eligibility`);
  }
  const blockedActivation = await fetch(`${baseUrl}/api/supply-registry/admin/entities/${encodeURIComponent(entityId)}/activate`, { method: 'POST', headers: auth(adminToken), body: '{}' });
  assert.equal(blockedActivation.status, 409, 'registry-verified supply entity cannot activate without a separately evidence-verified provider account');
