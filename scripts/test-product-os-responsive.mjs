...
try {
    ...
    for (const viewport of viewports) {
      ...
      for (const route of routes) {
        ...
        const evaluation = await evaluate(cdp, `(() => JSON.stringify({
          ...
          globalToolsPresent: ['k-reference-header-search','header-nearby-radar','header-points','header-cart','header-account'].every((id) => Boolean(document.getElementById(id))) && !document.getElementById('header-call'),
          launcherScoped: (() => { const inAgent = location.pathname === '/chat' || location.pathname === '/chat/'; const phoneLayout = innerWidth <= 767; const launcher = document.querySelector('.k-ask-kurukoo-launcher'); const launcherVisible = Boolean(launcher) && getComputedStyle(launcher).display !== 'none' && getComputedStyle(launcher).visibility !== 'hidden'; return inAgent || phoneLayout ? !launcherVisible : launcherVisible; })(),
          contextualSurfacePolicy: (() => { const broadCanvas = ['/desk', '/desk/', '/discover', '/discover/'].includes(location.pathname); const rail = document.querySelector('.k-reference-context-rail'); return !broadCanvas || !rail || getComputedStyle(rail).display === 'none'; })(),
          ...
          mobileAgentComposerGeometry: innerWidth <= 767 && (location.pathname === '/chat' || location.pathname === '/chat/') ? (() => { const composer = document.querySelector('.composer-wrap'); const tabs = document.querySelector('.k-mobile-tabbar'); if (!composer || !tabs) return { clear: false, reason: 'missing composer or tab bar' }; const composerRect = composer.getBoundingClientRect(); const tabsRect = tabs.getBoundingClientRect(); return { clear: composerRect.height >= 56 && composerRect.bottom <= tabsRect.top + 1, composerHeight: Math.round(composerRect.height), composerBottom: Math.round(composerRect.bottom), tabsTop: Math.round(tabsRect.top) }; })() : { clear: true }
        }))()`);
        ...
        const finalPass = pass && layout.hasShell && layout.referenceShell && layout.globalToolsPresent && layout.launcherScoped && layout.contextualSurfacePolicy && layout.agentCreationScoped && layout.desktopPaneModel && layout.tabletDrawerModel && layout.mobileMoreModel && layout.mobileMoreInteraction && layout.mobileAgentComposerGeometry.clear && layout.selectorVisible && layout.focusedAgent && !layout.horizontalOverflow && !layout.visibleError && textChecks.every((check) => check.pass);
  ...
  console.log(JSON.stringify({ summary: { total: results.length, passed: results.length - failed.length, failed: failed.length }, failures: failed.map((result) => ({ viewport: result.viewport, route: result.route, overflow: result.layout.horizontalOverflow, selectorVisible: result.layout.selectorVisible, referenceShell: result.layout.referenceShell, globalToolsPresent: result.layout.globalToolsPresent, agentCreationScoped: result.layout.agentCreationScoped, desktopPaneModel: result.layout.desktopPaneModel, tabletDrawerModel: result.layout.tabletDrawerModel, mobileMoreModel: result.layout.mobileMoreModel, mobileAgentComposerGeometry: result.layout.mobileAgentComposerGeometry, focusedAgent: result.layout.focusedAgent, missingText: result.textChecks.filter((check) => !check.pass).map((check) => check.expected) })) }, null, 2));
