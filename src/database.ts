...
export function seedCanonicalOperatorState(database: any): void {
  ...
  database.run(`CREATE TABLE IF NOT EXISTS agent_goals (id TEXT PRIMARY KEY, phone TEXT NOT NULL, conversation_id TEXT, economic_request_id TEXT, parent_goal_id TEXT, source TEXT NOT NULL, goal_type TEXT NOT NULL, objective TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'active', priority INTEGER NOT NULL DEFAULT 50, autonomy TEXT NOT NULL DEFAULT 'assist', next_action_at TEXT, completed_at TEXT, failure_reason TEXT, summary TEXT, plan_json TEXT, risk_level TEXT DEFAULT 'read_only', confirmation_required INTEGER DEFAULT 0, expires_at TEXT, created_at TEXT DEFAULT CURRENT_TIMESTAMP, updated_at TEXT DEFAULT CURRENT_TIMESTAMP)`);
