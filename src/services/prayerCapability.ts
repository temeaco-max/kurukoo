...
export function ensurePrayerCapability(): void {
  ...
  registerComposableSkillExtension({
    ...
    requiredInputs: [
      { key: 'topic', label: 'Prayer topic', required: false },
    ],
    optionalInputs: [
      { key: 'name', label: 'Name to include' },
      { key: 'tradition', label: 'Prayer tradition' },
      { key: 'tone', label: 'Prayer tone' },
      { key: 'length', label: 'Prayer length' },
      { key: 'dueAt', label: 'Routine time' },
      { key: 'recurrence', label: 'Routine recurrence' },
    ],
      ...
      execute: async (context) => {
        ...
        if (context.action === 'understand' || context.action === 'clarify') {
          return {
            status: 'needs_user',
            message: topic
              ? 'I can pray about that. Tell me the tradition or style you would like, or I can keep it general.'
        ...
        if (context.action === 'pray' || context.action === 'audio') {
          if (!topic) {
            return { status: 'needs_user', message: 'Tell me what you would like prayer for.' };
          }

          const rawLength = String(args.length || '');
          const length = ['short', 'medium', 'long'].includes(rawLength)
            ? (rawLength as 'short' | 'medium' | 'long')
            : 'medium';
          ...
          const result = await generatePrayer({
            ...
            tradition: args.tradition as any,
          ...
          return {
            ...
            canonicalFacts: {
              ...
              model: result.model,
              modality: context.action === 'audio' ? 'audio_ready' : 'text',
            },
            evidenceLevel: 'canonical_service',
            externalActivation: 'locally_available',
            nextActions: [
              { action: 'audio', label: 'Pray it aloud' },
              { action: 'routine', label: 'Make this a prayer routine' },
            ],
          };
        }
        ...
        if (context.action === 'routine') {
          if (!topic || !args.dueAt) {
            return {
              status: 'needs_user',
              message: 'Tell me what you want prayed for and when you want your prayer routine to begin.',
            };
          }
          ...
          const reminder = await createPrayerRoutine({
            ...
            recurrence: args.recurrence === 'weekly' ? 'weekly' : 'daily',
            tradition: args.tradition as any,
            name: args.name ? String(args.name) : undefined,
          });
          ...
          return {
            status: 'completed',
            message: `Your prayer routine is scheduled for ${reminder.due_at}.`,
            canonicalFacts: {
        ...
        return {
          status: 'needs_user',
          message: 'Tell me whether you want a prayer, spoken prayer, or a recurring prayer routine.',
        };
      },
    },
  });
