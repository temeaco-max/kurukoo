...
import { generatePrayer, createPrayerRoutine } from './prayerAgentService.js';
    ...
    executionAdapter: {
      ...
      execute: async (context) => {
        ...
        if (context.action === 'pray' || context.action === 'audio') {
          ...
          const result = await generatePrayer({ phone: context.phone, conversationId: context.conversationId, topic, name: args.name ? String(args.name) : undefined, tradition: args.tradition as any, tone: args.tone ? String(args.tone) : undefined, length: ['short', 'medium', 'long'].includes(String(args.length)) ? String(args.length) as any : 'medium' });
        ...
        if (context.action === 'routine') {
          ...
          const reminder = await createPrayerRoutine({ phone: context.phone, conversationId: context.conversationId, topic, dueAt: String(args.dueAt), recurrence: args.recurrence === 'weekly' ? 'weekly' : 'daily', tradition: args.tradition as any, name: args.name ? String(args.name) : undefined });
