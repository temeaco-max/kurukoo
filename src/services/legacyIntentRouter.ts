...
=======
  const outcomeSkill = explicitFoodOutcome ? 'order_food' : explicitAutomotiveServiceOutcome ? 'find_worker' : explicitTransportOutcome ? 'ride_request' : explicitAccommodationOutcome ? 'hotel_deals' : explicitHouseholdServiceOutcome ? 'find_worker' : explicitPropertyOutcome ? 'rental_tracker' : explicitJobOutcome ? 'job_tracker' : explicitTutorOutcome ? 'home_tutor' : explicitInternetServiceOutcome ? 'wifi_installer' : explicitRepairOutcome ? 'phone_repairer' : explicitProductOutcome ? 'product_sourcing' : explicitProviderOutcome ? 'find_worker' : null;
>>>>>>> df643e06 (feat(outcomes): route tutors through shared service)
...
=======
      const seed: Record<string, unknown> = directSkill === 'find_worker' && worker ? { service: worker } : (directSkill === 'repair' || directSkill === 'phone_repair' || directSkill === 'phone_repairer') ? { ...extractRepairSlots(query), ...(priorObservation ? { prior_diagnostics: priorObservation } : {}) } : directSkill === 'device_support' ? { problem: query.trim(), desired_action: 'diagnose' } : directSkill === 'product_sourcing' ? { product: query.trim() } : directSkill === 'verified_artist' ? { event_type: query.trim() } : directSkill === 'order_food' ? { ...(foodSlots.items ? { items: foodSlots.items } : {}), ...(foodSlots.location ? { location: foodSlots.location } : {}), ...extractFollowUpPatch(query, directSkill) } : directSkill === 'job_tracker' ? { objective: query.trim(), ...extractJobSearchSlots(query), ...extractFollowUpPatch(query, directSkill) } : ['hotel_deals','rental_tracker','home_tutor','wifi_installer'].includes(String(directSkill)) ? { objective: query.trim(), ...extractFollowUpPatch(query, directSkill) } : {};
>>>>>>> df643e06 (feat(outcomes): route tutors through shared service)
