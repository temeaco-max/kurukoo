...
async function handleExplicitOSAction(phone: string | undefined, q: string, threadId?: string, pointsEngineEnabled = true): Promise<IntentRoutingResult | null> {
  if (!phone || phone.startsWith('anon_')) return null;
...
export async function routeIntent(query: string, phone?: string, provider?: AIProvider, contextHint?: ConversationalContextHint, threadId?: string): Promise<IntentRoutingResult> {
  ...
  const osAction = explicitNotificationRead ? null : await handleExplicitOSAction(phone, q, threadId, pointsEngineEnabled);
