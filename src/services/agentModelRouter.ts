...
/** Cost-aware model selection only. It never executes work or grants authority. */
export function routeAgentModel(input: {
  ...
  if (requested) return { reasoningClass: input.reasoningClass, route: 'strong_remote', model: requested, rationale: 'explicit model selection' };
  ...
  return { reasoningClass: input.reasoningClass, route: 'strong_remote', model: 'configured-default', rationale: 'existing provider boundary remains responsible for default selection' };
}
