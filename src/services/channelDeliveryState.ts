...
export type ChannelDeliveryStatus = 'accepted' | 'submitted' | 'buffered' | 'delivered' | 'failed' | 'rejected' | 'unknown';
...
export async function recordChannelDispatch(input: { channel: string; provider: string; providerMessageId?: string; phone?: string; status?: string; raw?: Record<string, unknown> }): Promise<ChannelDeliveryState | null> {
...
export async function recordChannelDeliveryReport(input: { channel: string; provider: string; providerMessageId: string; phone?: string; status?: string; failureReason?: string; raw?: Record<string, unknown> }): Promise<ChannelDeliveryState | null> {
  const providerMessageId = String(input.providerMessageId || '').trim();
  if (!providerMessageId) return null;
  await ensureChannelDeliverySchema();
  const store = await getCanonicalStore();
...
export async function getChannelDeliveryState(channel: string, provider: string, providerMessageId: string): Promise<ChannelDeliveryState | null> {
  ...
  if (!row) return null;
  return {
    channel: String(row.channel), provider: String(row.provider), providerMessageId: String(row.provider_message_id),
    phone: row.phone ? String(row.phone) : undefined, status: normalizeProviderStatus(row.status),
    failureReason: row.failure_reason ? String(row.failure_reason) : undefined,
    raw: (() => { try { return JSON.parse(String(row.raw_json || '{}')); } catch { return {}; } })(),
    createdAt: String(row.created_at || ''), updatedAt: String(row.updated_at || ''),
  };
}

export function normalizeChannelDeliveryStatus(status: unknown): ChannelDeliveryStatus { return normalizeProviderStatus(status); }
