...
async function ensureTable() { const db = await getDb(); db.run(table()); return db; }
function idFor(phone: string, skill: string, kind: CapabilityKind) { return `${kind}:${phone}:${skill}`.slice(0, 220); }
function parseJson(value: unknown): Record<string, unknown> { if (!value) return {}; try { return typeof value === 'string' ? JSON.parse(value) : (value as Record<string, unknown>); } catch { return {}; } }
...
async function reconcileFromCanonicalSources(phone: string) {
  ...
  while (skillsStmt.step()) {
    ...
    db.run(`INSERT INTO capability_portfolio (id, phone, skill, kind, status, availability, metadata_json)
      VALUES (?, ?, ?, 'provider', ?, ?, '{}')
      ON CONFLICT(phone, skill, kind) DO UPDATE SET status=excluded.status, availability=excluded.availability, updated_at=CURRENT_TIMESTAMP`,
  ...
  profileStmt.free();
  if (contributor) db.run(`INSERT INTO capability_portfolio (id, phone, skill, kind, status, availability, metadata_json)
...
async function enrich(phone: string, rows: any[]): Promise<CapabilityPortfolioItem[]> {
  ...
  pulseStmt.bind([phone]); while (pulseStmt.step()) pulse.add(String((pulseStmt.getAsObject() as any).skill || '')); pulseStmt.free();
  const profileStmt = db.prepare(`SELECT verified_provider, trust_score FROM memory_profiles WHERE phone = ? LIMIT 1`);
  profileStmt.bind([phone]); let verified = false; let trustScore = 5;
  if (profileStmt.step()) { const row = profileStmt.getAsObject() as any; verified = Number(row.verified_provider || 0) === 1; trustScore = Number(row.trust_score || 5); }
  profileStmt.free();
  return rows.map((row: any) => {
    const skill = String(row.skill); const kind = (row.kind || 'provider') as CapabilityKind;
    ...
    return { id: String(row.id), phone, skill, kind, status, availability, category: getEconomicCategory(skill), confidence: Number(row.confidence || 1), operationMode: String(row.operation_mode || 'stationary'), serviceRadiusKm: Number(row.service_radius_km || 10), isVerified: verified || status === 'verified', trustScore, metadata: parseJson(row.metadata_json), activePulse: pulse.has(skill), requirements: getSkillRequirements(skill), capabilities: getSkillCapabilities(skill) };
  });
}
...
export async function listCapabilityPortfolio(phone: string): Promise<CapabilityPortfolioItem[]> {
  ...
  stmt.bind([phone]); const rows: any[] = []; while (stmt.step()) rows.push(stmt.getAsObject()); stmt.free();
...
export async function ensureCapability(phone: string, skill: string, kind: CapabilityKind = 'provider', metadata: Record<string, unknown> = {}): Promise<CapabilityPortfolioItem> {
  ...
  await ensureTable(); const db = await getDb();
  if (kind === 'provider') {
    const existingSkill = db.prepare(`SELECT id FROM skills WHERE phone = ? AND skill = ? LIMIT 1`); existingSkill.bind([phone, normalized]); const exists = existingSkill.step(); existingSkill.free();
  ...
  const existing = db.prepare(`SELECT id FROM capability_portfolio WHERE phone = ? AND skill = ? AND kind = ? LIMIT 1`); existing.bind([phone, normalized, kind]); const hasExisting = existing.step(); existing.free();
  if (!hasExisting) db.run(`INSERT INTO capability_portfolio (id, phone, skill, kind, status, availability, metadata_json) VALUES (?, ?, ?, ?, 'interested', 'offline', ?)` , [idFor(phone, normalized, kind), phone, normalized, kind, JSON.stringify(metadata)]);
  else if (Object.keys(metadata).length) db.run(`UPDATE capability_portfolio SET metadata_json = ?, updated_at = CURRENT_TIMESTAMP WHERE phone = ? AND skill = ? AND kind = ?`, [JSON.stringify(metadata), phone, normalized, kind]);
  saveDb();
  const item = (await listCapabilityPortfolio(phone)).find(x => x.skill === normalized && x.kind === kind); if (!item) throw new Error('Unable to create capability portfolio item'); return item;
}
...
export async function setCapabilityState(phone: string, skill: string, patch: { status?: CapabilityStatus; availability?: CapabilityAvailability; metadata?: Record<string, unknown>; kind?: CapabilityKind }) {
  ...
  const db = await getDb();
  if (kind === 'provider') {
    if (patch.availability === 'available' || patch.availability === 'offline') db.run(`UPDATE skills SET is_available = ? WHERE phone = ? AND skill = ?`, [patch.availability === 'available' ? 1 : 0, phone, normalized]);
    if (patch.status === 'paused' || patch.availability === 'offline') db.run(`UPDATE skills SET is_available = 0 WHERE phone = ? AND skill = ?`, [phone, normalized]);
  } else if (kind === 'contributor' && (patch.status === 'paused' || patch.availability === 'offline')) db.run(`UPDATE memory_profiles SET is_contributor = 0, updated_at = CURRENT_TIMESTAMP WHERE phone = ?`, [phone]);
  if (patch.status && kind !== 'provider') db.run(`UPDATE capability_portfolio SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [patch.status, current.id]);
...
export async function capabilityPortfolioSummary(phone: string) { const items = await listCapabilityPortfolio(phone); return { phone, total: items.length, active: items.filter(x => ['active','verified'].includes(x.status)).length, live: items.filter(x => x.availability === 'live').length, available: items.filter(x => x.availability === 'available' || x.availability === 'live').length, contributor: items.filter(x => x.kind === 'contributor').length, provider: items.filter(x => x.kind === 'provider').length, skills: items.map(x => x.skill), items }; }

export function isKnownSkill(skill: string) { const normalized = skill.trim().toLowerCase().replace(/\s+/g, '_'); return getKnownSkills().includes(normalized) || ['mobile_barber','barber','delivery_runner','prayer','life_admin','career','health_navigation','family_care','learning_tutor','home_household','finance_coach','grief_support','contributor'].includes(normalized); }
