...
export function ensureCapabilityPortfolioRegistration(): void {
  ...
  registerCapabilityExecutionAdapter(CAPABILITY_PORTFOLIO_CAPABILITY, {
    owner: 'capabilityPortfolioService',
    actions: ['inspect', 'add', 'update', 'pause', 'available', 'live', 'offline'],
    actionMetadata: {
      inspect: { risk: 'read_only', confirmationRequired: false },
      add: { risk: 'low_risk', confirmationRequired: false },
      update: { risk: 'low_risk', confirmationRequired: false },
      pause: { risk: 'low_risk', confirmationRequired: false },
      available: { risk: 'low_risk', confirmationRequired: false },
      live: { risk: 'confirmation_required', confirmationRequired: true },
      offline: { risk: 'low_risk', confirmationRequired: false },
    },
    execute: async (context) => {
      ...
      const kind = ['provider','contributor','native','agent'].includes(String(context.arguments.kind)) ? String(context.arguments.kind) as any : 'provider';
      if (context.action === 'inspect') return { status: 'completed', message: 'Capability portfolio inspected.', canonicalFacts: { capabilityPortfolio: await listCapabilityPortfolio(context.phone) }, evidenceLevel: 'canonical_service', externalActivation: 'locally_available', nextActions: ['add','update','inspect'] };
      if (!skill) return { status: 'needs_user', message: 'Tell me which capability skill to change.', nextActions: ['clarify','inspect'] };
      ...
      if (context.action === 'pause' || context.action === 'offline') { await endPulseSession(context.phone, skill); const capability = await setCapabilityState(context.phone, skill, { availability: 'offline', kind }); return { status: 'completed', message: `${skill.replace(/_/g, ' ')} is offline. Other capabilities are unchanged.`, canonicalFacts: { capability }, evidenceLevel: 'canonical_service', externalActivation: 'locally_available', nextActions: ['available','live','inspect'] }; }
      if (context.action === 'available' || context.action === 'update') { const capability = await setCapabilityState(context.phone, skill, { availability: context.action === 'available' ? 'available' : undefined, kind, metadata: typeof context.arguments.metadata === 'object' && context.arguments.metadata ? context.arguments.metadata as Record<string, unknown> : undefined }); return { status: 'completed', message: `${skill.replace(/_/g, ' ')} is updated on your capability portfolio. Verification and Pulse activation remain separate controls.`, canonicalFacts: { capability }, evidenceLevel: 'canonical_service', externalActivation: 'locally_available', nextActions: ['live','pause','inspect'] }; }
      if (context.action === 'live') { const lat = Number(context.arguments.lat); const lng = Number(context.arguments.lng); if (!Number.isFinite(lat) || !Number.isFinite(lng)) return { status: 'needs_user', message: 'I need your explicit current latitude and longitude before starting Nearby Pulse for this capability.', nextActions: ['clarify','cancel'] }; const pulse = await activatePulse(context.phone, skill, lat, lng); if (!pulse.success) return { status: 'external_unavailable', message: pulse.message, externalActivation: 'unavailable_external_dependency', nextActions: ['inspect','retry'] }; const capability = await setCapabilityState(context.phone, skill, { availability: 'live', status: 'active', kind }); return { status: 'completed', message: `${skill.replace(/_/g, ' ')} is live on Nearby Pulse.`, canonicalFacts: { capability, pulse }, evidenceLevel: 'canonical_service', externalActivation: 'locally_available', nextActions: ['offline','inspect'] }; }
      return null;
    },
  });
}
