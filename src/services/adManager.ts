...
async function ensureAdSchema() {
    ...
    await db.run(`CREATE TABLE IF NOT EXISTS ad_campaigns (
        ...
        region TEXT DEFAULT '',
        start_at ${pg ? 'TIMESTAMPTZ' : 'TEXT'},
        expires_at ${pg ? 'TIMESTAMPTZ' : 'TEXT'},
        frequency_cap INTEGER DEFAULT 3,
        impressions INTEGER DEFAULT 0,
        clicks INTEGER DEFAULT 0,
        created_at ${pg ? 'TIMESTAMPTZ' : 'TEXT'} DEFAULT CURRENT_TIMESTAMP,
        updated_at ${pg ? 'TIMESTAMPTZ' : 'TEXT'} DEFAULT CURRENT_TIMESTAMP,
        asset_status TEXT DEFAULT 'pending',
        approved_by TEXT DEFAULT ''
    )`);
    const columns = new Set<string>(pg
      ? (await db.all<{ column_name: string }>('SELECT column_name FROM information_schema.columns WHERE table_schema=current_schema() AND table_name=?', ['ad_campaigns'])).map(r => String(r.column_name))
      : (await db.all<any>('PRAGMA table_info(ad_campaigns)')).map(r => String(r.name ?? r[1]))
    );
    const additions: Array<[string, string]> = [
        ['campaign_type', "TEXT DEFAULT 'external'"], ['disclosure', "TEXT DEFAULT 'Sponsored'"], ['advertiser_name', "TEXT DEFAULT ''"],
        ['first_party', 'INTEGER DEFAULT 0'], ['cta_text', "TEXT DEFAULT 'Learn more'"], ['destination', "TEXT DEFAULT '/chat'"],
        ['priority', 'INTEGER DEFAULT 0'], ['targeting', "TEXT DEFAULT '{}'"], ['placement', "TEXT DEFAULT 'public_discovery'"],
        ['category', "TEXT DEFAULT 'community'"], ['country', "TEXT DEFAULT 'NG'"], ['region', "TEXT DEFAULT ''"],
        ['start_at', pg ? 'TIMESTAMPTZ' : 'TEXT'], ['expires_at', pg ? 'TIMESTAMPTZ' : 'TEXT'], ['frequency_cap', 'INTEGER DEFAULT 3'],
        ['impressions', 'INTEGER DEFAULT 0'], ['clicks', 'INTEGER DEFAULT 0'], ['asset_status', "TEXT DEFAULT 'pending'"], ['approved_by', "TEXT DEFAULT ''"],
    ];
    for (const [name, definition] of additions) {
      if (!columns.has(name)) await db.run(`ALTER TABLE ad_campaigns ADD COLUMN ${name} ${definition}`);
    }
    await db.run(`UPDATE ad_campaigns SET
      campaign_type=COALESCE(campaign_type,'external'), disclosure=COALESCE(disclosure,'Sponsored'), advertiser_name=COALESCE(advertiser_name,''),
      first_party=COALESCE(first_party,0), cta_text=COALESCE(cta_text,'Learn more'), destination=COALESCE(destination,'/chat'),
      priority=COALESCE(priority,0), targeting=COALESCE(targeting,'{}'), placement=COALESCE(placement,'public_discovery'),
      category=COALESCE(category,'community'), country=COALESCE(country,'NG'), region=COALESCE(region,''), frequency_cap=COALESCE(frequency_cap,3),
      impressions=COALESCE(impressions,0), clicks=COALESCE(clicks,0), asset_status=COALESCE(asset_status,CASE WHEN first_party=1 THEN 'approved' ELSE 'pending' END), approved_by=COALESCE(approved_by,'')`);
    return db;
}
...
function mapCampaign(row: Record<string, unknown>): AdCampaign {
  ...
  for (const [key, value] of Object.entries(row)) mapped[key.replace(/_([a-z])/g, (_, letter: string) => letter.toUpperCase())] = value;
  return mapped as AdCampaign;
}

export async function getAdCampaigns(): Promise<AdCampaign[]> {
    const db = await ensureAdSchema();
    return (await db.all<Record<string, unknown>>('SELECT * FROM ad_campaigns ORDER BY id ASC')).map(mapCampaign);
}

export interface RenderableCampaignOptions { firstParty?: boolean; placements?: string[]; limit?: number; now?: number; }
...
export interface UpdateAdCampaignInput {
    title?: string; desc?: string; imageUrl?: string; targetKeyword?: string; creditsBudget?: number; status?: 'active' | 'paused' | 'completed';
    campaignType?: string; disclosure?: string; advertiserName?: string; ctaText?: string; destination?: string; placement?: string; category?: string;
    country?: string; region?: string; startAt?: string | null; expiresAt?: string | null; frequencyCap?: number; priority?: number; targeting?: string;
    assetStatus?: 'pending' | 'approved' | 'rejected'; approvedBy?: string;
}
...
export async function updateAdCampaign(id: number, patch: UpdateAdCampaignInput, actor = 'admin'): Promise<AdCampaign> {
    ...
    const title = String(next.title || '').trim().slice(0, 160), desc = String(next.desc || '').trim().slice(0, 1000), imageUrl = String(next.imageUrl || '').trim().slice(0, 500), targetKeyword = String(next.targetKeyword || '').trim().toLowerCase().slice(0, 80), disclosure = String(next.disclosure || '').trim().slice(0, 120);
    ...
    await db.run(`UPDATE ad_campaigns SET ${assignments} WHERE id = ?`, [...Object.values(fields), id]);
...
export async function recordAdImpression(id: number): Promise<boolean> {
    if (!(await getCampaignForTracking(id))) return false;
    await (await ensureAdSchema()).run('UPDATE ad_campaigns SET impressions=COALESCE(impressions,0)+1, updated_at=? WHERE id=?',[new Date().toISOString(),id]);
    return true;
}
export async function recordAdClick(id: number): Promise<boolean> {
    if (!(await getCampaignForTracking(id))) return false;
    await (await ensureAdSchema()).run('UPDATE ad_campaigns SET clicks=COALESCE(clicks,0)+1, updated_at=? WHERE id=?',[new Date().toISOString(),id]);
    return true;
}
...
export async function createAdCampaign(campaign: Omit<AdCampaign, 'id' | 'creditsSpent' | 'status' | 'createdAt' | 'updatedAt'>): Promise<any> {
    ...
    await db.run(`INSERT INTO ad_campaigns (title,desc,image_url,target_keyword,credits_budget,campaign_type,disclosure,advertiser_name,first_party,cta_text,destination,placement,category,country,region,start_at,expires_at,frequency_cap,priority,targeting,asset_status,approved_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [campaign.title,campaign.desc,campaign.imageUrl,campaign.targetKeyword,campaign.creditsBudget,campaign.campaignType||'external',campaign.disclosure||'Sponsored',campaign.advertiserName||'',firstParty,campaign.ctaText||'Learn more',campaign.destination||'/chat',campaign.placement||'public_discovery',campaign.category||'community',campaign.country||'NG',campaign.region||'',campaign.startAt||null,campaign.expiresAt||null,campaign.frequencyCap||3,campaign.priority||0,campaign.targeting||'{}','approved',firstParty?'Kurukoo':'admin']);
    return { success: true, message: 'Ad campaign created successfully' };
}

export async function spendAdCampaign(id: number, cost = 2): Promise<boolean> {
    const db = await ensureAdSchema();
    const campaign = await db.one<any>('SELECT credits_budget,credits_spent FROM ad_campaigns WHERE id=?',[id]);
    if (!campaign) return false;
    const budget=Number(campaign.credits_budget||0),spent=Number(campaign.credits_spent||0),newSpent=spent+cost;
    if(newSpent>=budget) await db.run(`UPDATE ad_campaigns SET credits_spent=?,status='completed',updated_at=? WHERE id=?`,[budget,new Date().toISOString(),id]);
    else await db.run('UPDATE ad_campaigns SET credits_spent=?,updated_at=? WHERE id=?',[newSpent,new Date().toISOString(),id]);
    return true;
}
...
export async function matchAdCampaigns(query: string): Promise<AdCampaign[]> {
    ...
    return (await getRenderableCampaigns()).filter((campaign) => { const keyword = String(campaign.targetKeyword || '').toLowerCase().trim(); return Boolean(keyword) && cleanQuery.includes(keyword); });
}
...
export async function seedDemoAdCampaigns(): Promise<void> {
    ...
    for(const [title,desc,keyword,cta,destination,placement,category,priority,imageUrl] of campaigns){
      const existing=await db.one<any>('SELECT id FROM ad_campaigns WHERE title=? AND first_party=1 LIMIT 1',[title]);
      if(existing){await db.run("UPDATE ad_campaigns SET image_url=?,asset_status='approved',approved_by='Kurukoo',updated_at=? WHERE id=?",[imageUrl,new Date().toISOString(),Number(existing.id)]);continue;}
      await db.run(`INSERT INTO ad_campaigns(title,desc,image_url,target_keyword,credits_budget,status,campaign_type,disclosure,advertiser_name,first_party,cta_text,destination,placement,category,country,region,frequency_cap,priority,targeting,asset_status,approved_by) VALUES(?,?,?,?,0,'active','first_party','Kurukoo promotion','Kurukoo',1,?,?,?,?,?,'',5,?,?,'approved','Kurukoo')`,[title,desc,imageUrl,keyword,cta,destination,placement,category,priority,JSON.stringify({scope:'public',safetyExcluded:true})]);
    }
}
