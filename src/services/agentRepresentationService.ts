...
export async function getAgentRepresentation(agentId:string):Promise<AgentRepresentation>{
  await ensureCommercialSchema(); const db=await getDb(); const stmt=db.prepare(`SELECT owner_phone,skill,status FROM agent_delegations WHERE agent_id=? AND status='active' ORDER BY updated_at DESC LIMIT 1`); stmt.bind([agentId]); if(!stmt.step()){stmt.free();return{isDelegated:false,agentId};} const row=stmt.getAsObject() as any; stmt.free(); return {isDelegated:true,agentId,ownerPhone:String(row.owner_phone||''),skill:String(row.skill||''),status:String(row.status||'')};
}
