...
function extractSkills(message: string): string[] {
  return [...new Set(ALIASES.filter(([pattern]) => pattern.test(message)).map(([, skill]) => skill))];
}
...
function isCapabilityStatement(message: string): boolean {
  ...
  return (selfClaim.test(message) || explicitAvailability.test(message) || portfolioControl.test(message)) && extractSkills(message).length > 0;
}

function action(message: string): 'add' | 'available' | 'pause' {
...
export async function capabilityPortfolioFastPath(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    const changed: Array<{ skill: string; kind: 'provider' | 'contributor'; capability: any }> = [];

    for (const skill of skills) {
      const kind = skill === 'contributor' ? 'contributor' : 'provider';
      if (kind === 'provider') {
        ...
        if (intent === 'pause') db.run(`UPDATE skills SET is_available = 0 WHERE phone = ? AND skill = ?`, [phone, skill]);
        if (intent === 'available') db.run(`UPDATE skills SET is_available = 1 WHERE phone = ? AND skill = ?`, [phone, skill]);
      }
      const capability = await ensureCapability(phone, skill, kind, { source: 'conversation', statement: message.slice(0, 300) });
      changed.push({ skill, kind, capability });
    }
    saveDb();

    const labels = changed.map(item => item.skill.replace(/_/g, ' '));
    const reply = intent === 'pause'
      ? `${labels.join(', ')} ${labels.length === 1 ? 'is' : 'are'} now offline. Other Kurukoo capabilities are unchanged.`
      : intent === 'available'
        ? `${labels.join(', ')} ${labels.length === 1 ? 'is' : 'are'} marked available on your capability portfolio. Going Live nearby still requires explicit Pulse activation, verification and location consent for each skill.`
        : `I’ve added ${labels.join(', ')} to the same Kurukoo identity. Each capability keeps its own onboarding, verification, availability and presence state, so you can be a provider and contributor at the same time.`;
    ...
    writeSse(res, { type: 'done', fullReply: reply, cardData: { type: 'capability_portfolio', capabilities: changed, canonicalAction: 'capability_portfolio.update' }, diagnostics: { classificationSource: 'rules', intentConfidence: 0.99, canonicalAction: 'capability_portfolio.update', finalState: 'completed' } });
    writeSse(res, '[DONE]');
    res.end();
  } catch (error) {
    next(error);
