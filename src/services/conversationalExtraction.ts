...
function parseIssue(text: string): string | undefined {
  ...
  if (explicit) return clean(explicit);
  const issue = text.match(/\b(?:slow|slower|freezing|frozen|crashing|overheating|hot|virus|malware|infected|broken|damaged|stuck|offline|disconnected|no internet|weak signal|not charging|won't charge|battery|storage full)\b[^.!?]*/i)?.[0];
  return clean(issue);
}
...
export function extractConversationalEntities(text: string, intent?: string): ConversationalEntities {
  ...
  const entities: ConversationalEntities = {
    intent,
    skill,
    outcomeVerb: detectUserOutcomeVerb(query),
    subject: device || food.items || undefined,
    ...
    network: /\b(?:wifi|wi-fi|internet|router|modem|broadband|network|connection|signal)\b/i.test(query) ? clean(query.match(/\b(?:wifi|wi-fi|internet|router|modem|broadband|network|connection|signal)\b/i)?.[0]) : undefined,
