...
export type ExternalIntegrationId =
  ...
  | 'stripe' | 'mobile_money' | 'mcp' | 'gemini' | 'mistral' | 'groq' | 'openrouter' | 'huggingface'
...
export function getExternalIntegrationReadiness(env: NodeJS.ProcessEnv = process.env): ExternalIntegrationReadiness[] {
  ...
  return [
    ...
    record({ id: 'google_sheets', name: 'Google Sheets', category: 'sources', description: 'Spreadsheet source connection.', canonicalBoundary: 'No canonical provider adapter is registered.', implemented: false, contractTested: false, mockVerified: false, credentialsReady: false, recovery: 'Keep spreadsheet data outside Kurukoo or upload an owner-controlled export as an artifact until an adapter is registered.', activationChecklist: ['Register a least-privilege Sheets adapter.', 'Add owner authorization, callback validation, and deterministic contract coverage.', 'Verify a real read/write lifecycle before activation.'] }, env),
    record({ id: 'notion', name: 'Notion', category: 'sources', description: 'Workspace notes and database source connection.', canonicalBoundary: 'No canonical provider adapter is registered.', implemented: false, contractTested: false, mockVerified: false, credentialsReady: false, recovery: 'Use a user-controlled artifact export until a Notion adapter is registered.', activationChecklist: ['Register a scoped Notion OAuth adapter.', 'Implement owner binding, retry/revocation, and contract fixtures.', 'Verify live page/database access with a test workspace.'] }, env),
    record({ id: 'outlook', name: 'Microsoft Outlook', category: 'sources', description: 'Mailbox and attachment source connection.', canonicalBoundary: 'No canonical provider adapter is registered.', implemented: false, contractTested: false, mockVerified: false, credentialsReady: false, recovery: 'Use a user-controlled artifact export until a Microsoft adapter is registered.', activationChecklist: ['Register a least-privilege Microsoft Graph adapter.', 'Implement owner authorization, signed state, revocation, and fixtures.', 'Verify a real mailbox message and attachment flow.'] }, env),
    record({ id: 'onedrive', name: 'Microsoft OneDrive', category: 'sources', description: 'Owner-controlled file source connection.', canonicalBoundary: 'No canonical provider adapter is registered.', implemented: false, contractTested: false, mockVerified: false, credentialsReady: false, recovery: 'Use managed artifacts or a user-controlled export until a OneDrive adapter is registered.', activationChecklist: ['Register a least-privilege Microsoft Graph adapter.', 'Implement owner authorization, storage lifecycle, and contract fixtures.', 'Verify upload/open/revocation on a real test account.'] }, env),
    ...
    record({ id: 'openrouter', name: 'OpenRouter', category: 'ai', description: 'Optional hosted text provider connection.', canonicalBoundary: 'No canonical OpenRouter provider adapter is registered.', implemented: false, contractTested: false, mockVerified: false, credentialsReady: false, recovery: 'Use the existing canonical hosted-provider chain or deterministic fallback until an approved adapter exists.', activationChecklist: ['Register one canonical provider adapter and rollout flag.', 'Add deterministic request, failure and attribution contracts.', 'Verify real provider evidence before activation.'] }, env),
    record({ id: 'huggingface', name: 'Hugging Face', category: 'ai', description: 'Teacher/training and managed GPU Jobs boundary, separate from hosted inference availability.', canonicalBoundary: 'huggingfaceJobsBackend and teacher/training adapters', implemented: true, contractTested: true, mockVerified: true, credentialsReady: PRESENT(env.HF_TOKEN) || PRESENT(env.HUGGINGFACE_API_KEY) || PRESENT(env.HF_API_KEY), featureFlag: 'huggingface_jobs', recovery: 'Keep training blocked and hosted inference unavailable until the requested capability returns provider evidence.', activationChecklist: ['Configure a scoped token and private artifact repository.', 'Run non-billing preflight and approve bounded cost explicitly.', 'Verify job launch, monitor, retrieval, benchmark and fail-closed registry promotion.'] }, env),
