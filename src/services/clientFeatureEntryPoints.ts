...
const RAW_FEATURE_ENTRYPOINTS: ReadonlyArray<[string, string, string, string, string, FeatureEntrypoint['primarySurface']]> = [
  ...
  ['mobility','Ride and mobility','Rides use Discover plus the shared request/fulfilment fabric. Vehicle choice is optional; broadcast occurs after the request is ready and lead cost is charged at provider acceptance.','/discover','/chat?prompt=I%20need%20a%20ride','discover'],
  ['provider-communication','Provider communication','Accepted providers and customers communicate through the request-scoped WebRTC session, with sparse location and arrival-gated voice.','/channels','/app/call','connect'],
  ['agent-network','Kurukoo Agent Network','Become a POS, sales, referral or field-support agent; sell eligible Points, earn commission and surface locally when active.','/network','/app/agent-network','tasks'],
  ['points-topup','Points top-up','Top up Kurukoo Points directly or through an authorised network agent; credits settle only after verified payment evidence.','/explore/finance-tax','/app/top-up','wallet'],
  ['catalogue','Products and inventory','Provider, business, WhatsApp, store and affiliate inventory converges into one catalogue used by Chat and Discover.','/explore/classifieds','/app/catalogue','discover'],
