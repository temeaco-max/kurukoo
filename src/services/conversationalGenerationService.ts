...
/** Normal chat starts local. Hosted providers are used only for deliberate bypass or escalation. */
...
export async function generateConversationalResponse(input: ConversationalGenerationInput): Promise<ConversationalGenerationResult> {
  ...
  const semanticGuidance = [
    '--- LLM semantic interpretation (primary conversational understanding; never reveal) ---',
    `mode=${semanticInterpretation.mode}`,
    `speech_act=${semanticInterpretation.speechAct}`,
    `intent=${semanticInterpretation.intent}`,
    semanticInterpretation.skillHint ? `skill_hint=${semanticInterpretation.skillHint}` : '',
    semanticInterpretation.capabilityHint ? `capability_hint=${semanticInterpretation.capabilityHint}` : '',
    `explicit_authorization=${semanticInterpretation.explicitAuthorization}`,
    `requires_clarification=${semanticInterpretation.requiresClarification}`,
    semanticInterpretation.references.length ? `references=${JSON.stringify(semanticInterpretation.references)}` : '',
    semanticInterpretation.missingInformation.length ? `missing_information=${semanticInterpretation.missingInformation.join(', ')}` : '',
    semanticInterpretation.goalStatements.length ? `goal_statements=${semanticInterpretation.goalStatements.join(' | ')}` : '',
    semanticInterpretation.topicShift ? 'topic_shift=true' : '',
    semanticInterpretation.resumption ? 'resumption=true' : '',
  ].filter(Boolean).join('\n');
  ...
  // Local-first chat must escalate when SmolLM2 could not produce a real response.
  if (generationMode === 'generate' && conversationProvider === 'smollm2' && response.provider === 'Kurukoo Template') {
    ...
    if (provider !== 'smollm2') {
      try {
        const escalatedResponse = await queryUnifiedAI(input.prompt, { provider, systemPrompt: contextualSystemPrompt || input.systemPrompt, phone: input.phone, threadId: input.threadId, conversational: true, contextHint: input.contextHint });
        if (escalatedResponse.provider !== 'Kurukoo Template' && escalatedResponse.text.trim()) {
  ...
  if (generationMode !== 'deterministic') {
    ...
    if (needsRepair && contract.mode !== 'control') {
      ...
      try {
        const repaired = await queryUnifiedAI(input.prompt, { provider, systemPrompt: [contextualSystemPrompt || input.systemPrompt || '', buildRepairInstruction(contract, assessment)].filter(Boolean).join('\n\n'), phone: input.phone, threadId: input.threadId, conversational: true, contextHint: input.contextHint });
        ...
        if (!responseViolatesActionPosture(repaired.text, contract) && (repairedAssessment.score >= assessment.score || repairedAssessment.conversational)) {
          ...
          escalated = escalated || repaired.provider !== base.provider || repaired.model !== base.model;
        }
      } catch {}
    }
    if (responseViolatesActionPosture(response.text, contract) && contract.mode !== 'control') {
      const provider = strongerProvider(conversationProvider);
      try {
        const strictRepair = await queryUnifiedAI(input.prompt, { provider, systemPrompt: [contextualSystemPrompt || input.systemPrompt || '', buildStrictRepairInstruction(contract)].filter(Boolean).join('\n\n'), phone: input.phone, threadId: input.threadId, conversational: true, contextHint: input.contextHint });
  ...
  if (routing) {
    const providers: AiProvider[] = ['mistral','gemini','groq','openrouter','poolside'];
