...
import { queryUnifiedAI, resolveConfiguredHostedProvider, resolveHostedProviderCandidates, getLastAiRoutingDiagnostic, type AIProvider, type AIResponse, type ConversationalContextHint } from './unifiedAiEngine.js';
...
const NON_COMPLEX_ESCALATION_PROVIDERS = new Set<AIProvider>(['mistral', 'gemini', 'groq', 'openrouter']);

/** Normal chat starts with SmolLM2. Explicit hosted selection remains possible for deliberate provider tests or escalation. */
...
function strongerProvider(preferred: AIProvider | undefined): AIProvider {
  if (preferred && preferred !== 'auto' && preferred !== 'smollm2' && preferred !== 'local_intent') return preferred;
  const hosted = resolveHostedProviderCandidates('auto');
  return hosted.find(provider => NON_COMPLEX_ESCALATION_PROVIDERS.has(provider)) || hosted[0] || resolveConfiguredHostedProvider() || 'smollm2';
}
...
function needsCanonicalRelevanceFallback(prompt: string, responseText: string): boolean { const user = String(prompt || '').toLowerCase(); const reply = String(responseText || '').toLowerCase(); return /\bwhat should i know before using kurukoo\b/.test(user) && /\b(?:user has not previously used kurukoo|ensure you have the following knowledge)\b/.test(reply); }
function canonicalRelevanceFallback(prompt: string, context?: { activeGoal?: string; activeGoals?: string[]; pendingFields?: string[]; knownFacts?: string[]; canonicalAction?: string; cardType?: string }): string { if (/\bwhat should i know before using kurukoo\b/i.test(prompt)) return 'Kurukoo can help you talk through everyday questions, keep useful context, set reminders, and coordinate requests. It will ask for the details needed before an action, and it will not claim a provider, price, payment, or completion without evidence.'; if (context) { const label = context.activeGoal || (context.activeGoals?.[0]) || ''; const pending = Array.isArray(context.pendingFields) && context.pendingFields.length ? context.pendingFields[0] : ''; const known = Array.isArray(context.knownFacts) && context.knownFacts.length ? context.knownFacts[0] : ''; if (label && pending) return `I’m still working on ${label}. I need this from you: ${pending}.`; if (label && known) return `I’m continuing ${label}. From what you’ve told me: ${known}. What would you like to do next for it?`; if (label) return `I’m still working on ${label}. Tell me the detail that matters most and I’ll keep the next step clear and safe.`; if (known) return `I remember you mentioned: ${known}. Tell me the one detail that matters most and I’ll keep the next step clear and safe.`; } return 'I’m here to help with the point you raised. Tell me the one detail that matters most, and I’ll keep the next step clear and safe.'; }
...
export async function generateConversationalResponse(input: ConversationalGenerationInput): Promise<ConversationalGenerationResult> {
  ...
  const conversationProvider = input.provider && input.provider !== 'auto' ? input.provider : decision.provider;
  ...
  if (generationMode === 'generate' && conversationProvider === 'smollm2' && response.provider === 'Kurukoo Template') {
    const provider = strongerProvider('smollm2');
    if (provider !== 'smollm2') { try { const escalatedResponse = await queryUnifiedAI(input.prompt, { provider, systemPrompt: contextualSystemPrompt || input.systemPrompt, phone: input.phone, threadId: input.threadId, conversational: true, contextHint: input.contextHint }); if (escalatedResponse.provider !== 'Kurukoo Template' && escalatedResponse.text.trim()) { response = escalatedResponse; assessment = assess(input, contract, response.text); attempts += 1; escalated = true; } } catch {} }
  }
  if (generationMode === 'generate' && needsCanonicalRelevanceFallback(input.prompt, response.text)) { response = { ...response, provider: 'Kurukoo Template', model: 'template-fallback', text: canonicalRelevanceFallback(input.prompt, { activeGoal: input.currentGoal, activeGoals: input.activeGoals, pendingFields: input.pendingFields, knownFacts: input.knownFacts, canonicalAction: input.canonicalAction, cardType: input.cardType }), cost: '$0.00' }; assessment = assess(input, contract, response.text); attempts += 1; escalated = true; }
  if (generationMode !== 'deterministic') {
    ...
    if (needsRepair && contract.mode !== 'control') { const provider = strongerProvider(conversationProvider); try { const repaired = await queryUnifiedAI(input.prompt, { provider, systemPrompt: [contextualSystemPrompt || input.systemPrompt || '', buildRepairInstruction(contract, assessment)].filter(Boolean).join('\n\n'), phone: input.phone, threadId: input.threadId, conversational: true, contextHint: input.contextHint }); const repairedAssessment = assess(input, contract, repaired.text); attempts += 1; if (!responseViolatesActionPosture(repaired.text, contract) && (repairedAssessment.score >= assessment.score || repairedAssessment.conversational)) { response = repaired; assessment = repairedAssessment; escalated = escalated || repaired.provider !== base.provider || repaired.model !== base.model; } } catch {} }
  ...
  if (routing) { const providers: AiProvider[] = ['mistral','gemini','groq','openrouter','poolside']; for (const provider of providers) { if (!routing.attemptedProviders.includes(provider)) continue; if (routing.actualProvider.toLowerCase().startsWith(provider)) recordAiProviderSuccess(provider, response.latencyMs); else recordAiProviderFailure(provider, routing.fallbackReason || 'provider_fallback', response.latencyMs); } }
  await recordAiUsageEvent({ requestId: input.threadId ? `${input.threadId}:${Date.now()}` : undefined, phone: input.phone, agentId: input.agentId, provider: response.provider, model: response.model, inputTokens: Math.ceil((contextualSystemPrompt.length + input.prompt.length) / 4), outputTokens: Math.ceil((response.text || '').length / 4), latencyMs: response.latencyMs, success: response.provider !== 'Kurukoo Template', escalationReason: escalated ? (decision.escalationReason || 'local_first_hosted_escalation') : decision.escalationReason });
