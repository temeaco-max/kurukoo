...
function fallbackInterpretation(input: SemanticConversationInput): SemanticConversationInterpretation {
  ...
  return { mode, speechAct: greeting ? 'greeting' : explicitAuthorization ? 'instruction' : clarification ? 'question' : mode === 'exploration' ? 'question' : 'conversation', intent: mode === 'action' ? 'action_request' : 'general_conversation', explicitAuthorization, requiresClarification: clarification, topicShift: /^(?:actually|by the way|btw|different question|separately|unrelated|also|one more thing)\b/i.test(text), resumption: reference, references: [], entities: {}, missingInformation: [], goalStatements: [], responseStrategy: mode === 'action' ? 'propose' : mode === 'clarification' ? 'clarify' : mode === 'control' ? 'control' : 'answer', confidence: 0.42, source: 'fallback' };
}
function sanitize(value: Record<string, unknown>, input: SemanticConversationInput, ai: { provider?: string; model?: string }): SemanticConversationInterpretation {
  ...
  const references = Array.isArray(value.references) ? value.references.slice(0, 8).map(item => { if (!item || typeof item !== 'object') return null; const row = item as Record<string, unknown>; return { text: cleanString(row.text, 160) || '', target: cleanString(row.target, 180), confidence: normalizeConfidence(row.confidence) }; }).filter(item => item && item.text) as Array<{ text: string; target?: string; confidence: number }> : [];
  ...
  return { mode, speechAct: requestedAct, intent: cleanString(value.intent, 120) || fallback.intent, skillHint: cleanString(value.skillHint, 100), capabilityHint: cleanString(value.capabilityHint, 140), explicitAuthorization, requiresClarification, topicShift: Boolean(value.topicShift), resumption: Boolean(value.resumption), references, entities, missingInformation: cleanStringArray(value.missingInformation), goalStatements: cleanStringArray(value.goalStatements), responseStrategy, confidence: normalizeConfidence(value.confidence), rationale: cleanString(value.rationale, 320), source: 'llm', provider: ai.provider, model: ai.model };
}
...
export async function interpretConversationSemantics(input: SemanticConversationInput): Promise<SemanticConversationInterpretation> {
  ...
  try {
    ...
    const ai = await queryUnifiedAI(semanticPrompt(input, contextPack.transcript || ''), { provider: decision.provider, phone: input.phone, threadId: input.threadId, conversational: false, useFastText: false, contextHint: input.contextHint, temperature: 0, skipMemory: true });
