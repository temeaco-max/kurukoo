...
async function ensureReminderSchema(): Promise<CanonicalStore> {
  const s = await getCanonicalStore();
  const pg = getCanonicalPersistenceMode() === 'postgres';
  const ts = pg ? 'TIMESTAMPTZ' : 'TEXT';
  await s.run(`CREATE TABLE IF NOT EXISTS reminders (id TEXT PRIMARY KEY, phone TEXT NOT NULL, title TEXT NOT NULL, note TEXT NOT NULL DEFAULT '', due_at ${ts} NOT NULL, recurrence TEXT, status TEXT NOT NULL DEFAULT 'scheduled', created_at ${ts} DEFAULT CURRENT_TIMESTAMP, sent_at ${ts}, source_conversation_id TEXT, resume_context_id TEXT)`);
  for (const column of ['source_conversation_id', 'resume_context_id']) {
    try { await s.run(`ALTER TABLE reminders ADD COLUMN ${column} TEXT`); } catch {}
  }
  await s.run('CREATE INDEX IF NOT EXISTS idx_reminders_phone_due ON reminders(phone, due_at, status)');
  await s.run('CREATE INDEX IF NOT EXISTS idx_reminders_due ON reminders(due_at, status)');
  return s;
}
...
export async function createReminder(phone: string, input: { title: string; note?: string; dueAt: string; recurrence?: string | null; sourceConversationId?: string | null; resumeContextId?: string | null }): Promise<Reminder> {
  const s = await ensureReminderSchema();
  ...
  await s.run(`INSERT INTO reminders (id, phone, title, note, due_at, recurrence, status, source_conversation_id, resume_context_id) VALUES (?, ?, ?, ?, ?, ?, 'scheduled', ?, ?)`, [id, phone, input.title.trim(), String(input.note || '').trim(), due.toISOString(), input.recurrence || null, input.sourceConversationId || null, input.resumeContextId || null]);
  try {
    const profile = await s.one<any>('SELECT behavior_patterns FROM memory_profiles WHERE phone = ?', [phone]);
    if (profile) { let patterns: any = {}; try { patterns = JSON.parse(String(profile.behavior_patterns || '{}')); } catch {} patterns.last_reminder_created = new Date().toISOString(); patterns.reminder_count = (Number(patterns.reminder_count) || 0) + 1; await s.run('UPDATE memory_profiles SET behavior_patterns = ?, updated_at = CURRENT_TIMESTAMP WHERE phone = ?', [JSON.stringify(patterns), phone]); }
  } catch {}
  const row = await s.one<any>('SELECT * FROM reminders WHERE id = ?', [id]); if (!row) throw new Error('Reminder could not be created');
...
export async function listReminders(phone: string, includeCompleted = false): Promise<Reminder[]> {
  ...
  return (await s.all<any>(sql, [phone])).map(rowToReminder);
}
...
export async function getReminderForPhone(phone: string, id: string): Promise<Reminder | null> {
  const s = await ensureReminderSchema();
  const row = await s.one<any>('SELECT * FROM reminders WHERE id = ? AND phone = ? LIMIT 1', [id, phone]);
  return row ? rowToReminder(row) : null;
}

export async function cancelReminder(phone: string, id: string): Promise<boolean> {
  const s = await ensureReminderSchema();
  await s.run("UPDATE reminders SET status = 'cancelled' WHERE id = ? AND phone = ? AND status = 'scheduled'", [id, phone]);
  const row = await s.one<any>("SELECT * FROM reminders WHERE id = ? AND phone = ? AND status = 'cancelled' LIMIT 1", [id, phone]); if (!row) return false;
...
export async function processDueReminders(limit = 100): Promise<{ checked: number; delivered: number; queued: number; failed: number }> {
  const s = await ensureReminderSchema(); const now = new Date().toISOString();
  const rows = await s.all<any>(`SELECT * FROM reminders WHERE status = 'scheduled' AND due_at <= ? ORDER BY due_at ASC LIMIT ${Math.max(1, Math.min(500, Math.floor(limit)))}`, [now]);
  let delivered = 0, queued = 0, failed = 0;
  for (const row of rows) {
    const reminder = rowToReminder(row); const body = reminder.note ? `${reminder.title} — ${reminder.note}` : reminder.title; let pushSent = false;
    ...
    await s.run(`INSERT INTO messages (phone, sender, content, channel, card_data, status) VALUES (?, 'kurukoo', ?, 'pwa', ?, ?)`, [reminder.phone, `Reminder: ${body}`, JSON.stringify(reminderCard), pushSent ? 'sent' : 'queued']);
    if (pushSent) delivered += 1; else queued += 1;
    if (reminder.recurrence) { const next = nextOccurrence(reminder.due_at, reminder.recurrence); if (next) { await s.run("UPDATE reminders SET due_at = ?, status = 'scheduled', sent_at = ? WHERE id = ?", [next, new Date().toISOString(), reminder.id]); reminder.due_at = next; reminder.status = 'scheduled'; reminder.sent_at = new Date().toISOString(); } else { await s.run("UPDATE reminders SET status = 'sent', sent_at = ? WHERE id = ?", [new Date().toISOString(), reminder.id]); reminder.status = 'sent'; reminder.sent_at = new Date().toISOString(); } } else { await s.run("UPDATE reminders SET status = 'sent', sent_at = ? WHERE id = ?", [new Date().toISOString(), reminder.id]); reminder.status = 'sent'; reminder.sent_at = new Date().toISOString(); }
