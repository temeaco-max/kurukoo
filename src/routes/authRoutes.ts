...
export async function upsertProfile(phone: string, name?: string, email?: string, goal?: string): Promise<void> {
  ...
  if (exists) {
    ...
    db.run(`INSERT INTO memory_profiles (phone, name, location, country, subscription_tier, wallet_balance_minor, preferences, behavior_patterns, email, is_available) VALUES (?, ?, 'Ibadan', 'ng', 'Base', 30, ?, '{}', ?, 1)`, 
      [phone, name || 'Kurukoo User', JSON.stringify({ goal: goal || 'buyer', onboarding_complete: true, onboarding_step: 'done' }), email || '']);
  }
  saveDb();
}
...
router.post('/verify-otp', authRateLimit, async (req, res) => {
  try {
    ...
    const secure = process.env.NODE_ENV === 'production' ? '; Secure' : '';
    const sessionCookies = [`kurukoo_auth=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000${secure}`];
...
router.post('/login', authRateLimit, async (req, res) => {
  try {
    ...
    if (process.env.OTP_LEGACY_LOGIN === 'true') {
      ...
      return res.json({ success: true, phone: userPhone, name: name || 'Kurukoo User', token, warning: 'Legacy login enabled' });
    }
