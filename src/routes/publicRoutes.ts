...
function getLocale(lang = 'en') { const localePath=path.join(process.cwd(),'locales',`${lang}.json`); const fallbackPath=path.join(process.cwd(),'locales','en.json'); try{const filePath=fs.existsSync(localePath)?localePath:fallbackPath;if(fs.existsSync(filePath))return JSON.parse(fs.readFileSync(filePath,'utf-8'));}catch{}return {tagline:'Tell Kurukoo what you need. Kurukoo figures out who or what can fulfil it.'}; }
...
function loadExploreCategories():ExploreCategory[]{try{const p=path.join(process.cwd(),'content','explore','categories.json');return fs.existsSync(p)?JSON.parse(fs.readFileSync(p,'utf-8')):[];}catch{return [];}}
function resolveExploreCategory(slug:string,categories:ExploreCategory[]){const canonical=EXPLORE_ALIASES[slug]||slug;return categories.find(category=>category.slug===canonical);}
...
>>>>>>> origin/feat/kurukoo-conversation-workspace
 ...
 router.get('/help',async(req,res,next)=>{try{await renderPage(req,res,'help','/help',{articles:[{title:'Getting Started',body:'Sign up with your phone number and start chatting.'},{title:'For Providers',body:'Add your skills to start accepting gigs.'},{title:'Payments',body:'Manage your Points and payment evidence.'}]});}catch(error){next(error);}});
