...
function withRelatedCategories(category: ExploreCategory, categories: ExploreCategory[]) {
  return { 
    ...category, 
    subCategories: (category.relatedSlugs || []).map(slug => categories.find(candidate => candidate.slug === slug)).filter((value): value is ExploreCategory => Boolean(value)) 
  };
}
...
export function createPublicRouter(): Router {
  ...
  const renderPage = async (req: express.Request, res: express.Response, view: string, reqPath = req.path, extra: Record<string, unknown> = {}) => {
    ...
    res.render(view, { country, t, shortcode: '*7000#', seo, schemas, faqs, reqPath, ...extra });
  };

  router.get('/', async (req, res, next) => {
    try {
      await renderPage(req, res, 'index', '/', { pageStyles: ['/css/kurukoo-home.css?v=1.0.0'] });
    } catch (error) {
      next(error);
    }
  });
  ...
  router.get('/explore/:slug', async (req, res, next) => {
    try {
      ...
      const ads = await getAdCampaigns();
      await renderPage(req, res, 'explore/category', `/explore/${req.params.slug}`, { category: withRelatedCategories(category, categories), ads });
    } catch (error) {
      next(error);
    }
  });
  ...
  router.get('/p/:providerSlug', async (req, res, next) => {
    try {
      ...
      const provider = {
        ...
        skills: skills.map((s: any) => ({ skill: String(s.skill || ''), rating: Number(s.rating) || 5, jobsCompleted: Number(s.jobs_completed) || 0, hourlyRate: Number(s.hourly_rate) || 0 }))
      };
  ...
  router.get('/web', optionalAuthenticateUser, (req: AuthRequest, res) => {
    if (!req.user?.phone) return res.redirect(302, '/login?return=%2Fweb');
    res.sendFile(path.join(process.cwd(), 'public', 'dashboard.html'));
  });

  router.get('/settings', optionalAuthenticateUser, (req: AuthRequest, res, next) => {
    if (!req.user?.phone) return res.redirect(302, '/login?return=%2Fsettings');
    // Prefer server-rendered settings page for better form handling
    renderPage(req, res, 'settings', '/settings').catch(next);
  });
    ...
    try {
      await renderPage(req, res, 'help', '/help', { 
        articles: [
          ...
          { title: 'Payments', body: 'Manage your Points and payment evidence.' }
        ] 
      });
    } catch (error) {
      next(error);
  ...
  router.get('/legal/:section?', async (req, res, next) => {
    try {
      await renderPage(req, res, 'legal', '/legal', { section: (req.params as any).section || 'privacy' });
    } catch (error) {
      next(error);
    }
  });
  ...
  router.get('/login', optionalAuthenticateUser, async (req: AuthRequest, res, next) => {
    try {
      const returnTo = typeof req.query.return === 'string' ? req.query.return : '';
      if (req.user?.phone) {
        return res.redirect(302, returnTo || '/web');
      }
      const conversationId = typeof req.query.conversationId === 'string' ? req.query.conversationId : '';
      let guestId = typeof req.query.guest_id === 'string' ? req.query.guest_id : '';
      if (!guestId) {
      ...
      await renderPage(req, res, 'login', '/login', { returnTo, conversationId, guestId, pageStyles: ['/css/kurukoo-auth.css?v=1.0.0'] });
    } catch (error) {
      next(error);
    }
  });
  ...
  router.get('/resources/:slug', async (req, res, next) => {
    try {
      await renderPage(req, res, 'resources/article', `/resources/${req.params.slug}`, { slug: req.params.slug });
    } catch (error) {
      next(error);
    }
  });
  ...
  router.get('/:country(ng|gh|gb)', async (req, res, next) => {
    try {
      const country = String((req.params as any).country);
      await renderPage(req, res, 'index', `/${country}`, { country, pageStyles: ['/css/kurukoo-home.css?v=1.0.0'] });
    } catch (error) {
      next(error);
    }
  });
