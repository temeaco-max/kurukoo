...
export function createPublicRouter():Router{const router=express.Router();
 ...
 router.get('/explore',async(req,res,next)=>{try{const categoriesPath=path.join(process.cwd(),'content','explore','categories.json');let categories:any[]=[];if(fs.existsSync(categoriesPath))categories=JSON.parse(fs.readFileSync(categoriesPath,'utf-8'));const ads=await getAdCampaigns();await renderPage(req,res,'explore/index','/explore',{categories,ads});}catch(error){next(error);}});
