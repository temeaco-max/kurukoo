...
function getLocale(lang = 'en') {
  ...
  return { tagline: 'Tell Kurukoo what you need. Kurukoo figures out who or what can fulfil it.' };
}
...
const EXPLORE_ALIASES: Record<string, string> = { groceries:'food', errands:'errands-delivery', 'repairs-maintenance':'repairs', 'solar-energy':'professional-services', 'automotive-mechanics':'automotive-care', 'health-medical':'health', 'emergency-dispatch':'emergency', 'reach-reference':'professional-services', sports:'fitness', 'sports-recreation':'fitness', travel:'tourism', 'events-entertainment':'events', 'price-check':'price-checker', cravings:'food' };
const EARN_CATEGORY: Record<string, string> = { refer:'money-circle', agents:'professional-services', rides:'transport', delivery:'logistics', repairs:'repairs', errands:'errands-delivery', support:'professional-services', build:'workers', work:'gigs', help:'professional-services', sell:'classifieds', promote:'professional-services', contributor:'gigs', tasks:'gigs' };
...
function resolveExploreCategory(slug: string, categories: ExploreCategory[]) { return categories.find(category => category.slug === (EXPLORE_ALIASES[slug] || slug)); }
function withRelatedCategories(category: ExploreCategory, categories: ExploreCategory[]) { return { ...category, subCategories: (category.relatedSlugs || []).map(slug => categories.find(candidate => candidate.slug === slug)).filter((value): value is ExploreCategory => Boolean(value)) }; }

const WORKSPACE_META: Record<string, { title: string; description: string }> = {
  requests: { title:'Requests', description:'Follow the things you have asked Kurukoo to help you get done.' },
  reminders: { title:'Reminders', description:'Keep important follow-ups connected to your Kurukoo conversations.' },
  saved: { title:'Saved & offers', description:'Keep useful options, offers and request context close at hand.' },
  cart: { title:'Cart', description:'Review sourced items before you request them.' },
  points: { title:'Points', description:'See your Points balance and eligible activity for this deployment.' },
  tasks: { title:'Tasks', description:'Contribute small pieces of useful information to the Kurukoo network.' },
  memory: { title:'Memory', description:'Review the useful context you choose to keep connected to Kurukoo.' },
  safety: { title:'Safety & urgent help', description:'Manage safety context and find the right next action without presenting Kurukoo as an emergency service.' },
  settings: { title:'Settings', description:'Manage your account, notifications, privacy, memory, channels and security.' }
};

export function createPublicRouter(): Router {
  const router = express.Router();
  const renderPage = async (req: express.Request, res: express.Response, view: string, reqPath = req.path, extra: Record<string, unknown> = {}) => {
    const country = typeof extra.country === 'string' ? extra.country : 'ng';
    const t = getLocale('en');
    const { seo, schemas, faqs } = await fetchSeoData(reqPath);
    // The legacy shortcode is deliberately empty: channel availability is deployment-specific and must not be implied by page rendering.
    res.render(view, { country, t, shortcode: '', seo, schemas, faqs, reqPath, ...extra });
  };

  const workspaceRoute = (section: keyof typeof WORKSPACE_META) => async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    try {
      const authReq = req as AuthRequest;
      if (!authReq.user?.phone) return res.redirect(302, `/login?return=${encodeURIComponent(req.path)}`);
      const meta = WORKSPACE_META[section];
      await renderPage(req, res, 'workspace', req.path, { workspaceSection: section, workspaceTitle: meta.title, workspaceDescription: meta.description, displayName: authReq.user.name || authReq.user.phone, phone: authReq.user.phone, pageStyles: ['/css/kurukoo-workspace.css?v=3'] });
  ...
  router.get('/p/:providerSlug', async (req,res,next)=>{ try { const db=await getDb(); const stmt=db.prepare('SELECT * FROM memory_profiles WHERE profile_slug = ?'); stmt.bind([req.params.providerSlug]); let profile:any=null; if(stmt.step())profile=stmt.getAsObject(); stmt.free(); if(!profile)return next(); const sStmt=db.prepare('SELECT * FROM skills WHERE phone = ?'); sStmt.bind([profile.phone]); const skills:any[]=[]; while(sStmt.step())skills.push(sStmt.getAsObject()); sStmt.free(); if(!skills.length)return next(); const providerName=String(profile.display_name||profile.name||'Provider'); const provider={name:providerName,initials:providerName.charAt(0).toUpperCase(),location:String(profile.location||profile.primary_lga||'Nigeria'),verified:profile.verified_provider===1,trustScore:Number(profile.trust_score)||5,skills:skills.map((s:any)=>({skill:String(s.skill||''),rating:Number(s.rating)||5,jobsCompleted:Number(s.jobs_completed)||0,hourlyRate:Number(s.hourly_rate)||0}))}; const reqPath=`/p/${req.params.providerSlug}`; const {seo,schemas,faqs}=await fetchSeoData(reqPath,providerName); res.render('provider-profile',{country:'ng',t:getLocale('en'),shortcode:'',provider,seo,schemas,faqs,reqPath}); } catch(error){console.error('Provider profile error:',error);next(error);} });

  router.get('/web', optionalAuthenticateUser, (req: AuthRequest, res) => { if(!req.user?.phone)return res.redirect(302,'/login?return=%2Fweb'); res.redirect(302,'/requests'); });
  router.get('/requests', optionalAuthenticateUser, workspaceRoute('requests'));
  router.get('/reminders', optionalAuthenticateUser, workspaceRoute('reminders'));
  router.get('/saved', optionalAuthenticateUser, workspaceRoute('saved'));
  router.get('/cart', optionalAuthenticateUser, workspaceRoute('cart'));
  router.get('/points', optionalAuthenticateUser, workspaceRoute('points'));
  router.get('/tasks', optionalAuthenticateUser, workspaceRoute('tasks'));
  router.get('/memory', optionalAuthenticateUser, workspaceRoute('memory'));
  router.get('/safety', optionalAuthenticateUser, workspaceRoute('safety'));
  router.get('/settings', optionalAuthenticateUser, workspaceRoute('settings'));
  router.get('/chat', async (_req,res)=>res.sendFile(path.join(process.cwd(),'public','chat','index.html')));

  router.get('/admin/login', async(req,res,next)=>{try{await renderPage(req,res,'admin/login','/admin/login',{pageStyles:['/css/admin-auth.css?v=1.0.0']});}catch(error){next(error);}});
  ...
  router.get('/earn/:topic',async(req,res,next)=>{try{const topic=req.params.topic;await renderPage(req,res,'earn',`/earn/${topic}`,{topic,category:EARN_CATEGORY[topic]||'gigs'});}catch(error){next(error);}});
  ...
  router.get('/help',async(req,res,next)=>{try{await renderPage(req,res,'help','/help',{articles:[{title:'Getting started',body:'Open Web Chat and tell Kurukoo what you need.'},{title:'For providers',body:'Join the network with the capabilities you can genuinely provide.'},{title:'Payments',body:'Payment and escrow states are shown only when the required deployment integrations are configured.'}]});}catch(error){next(error);}});
