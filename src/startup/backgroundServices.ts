...
import { startCommunicationOutboxWorker } from '../services/communicationOutbox.js';
...
export async function startBackgroundServices(): Promise<void> {
    ...
    try { startCommunicationOutboxWorker(); } catch (error) { console.error('Failed to start communication outbox worker:', error); }
