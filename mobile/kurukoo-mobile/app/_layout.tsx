...
function OfflineQueueObserver() {
  useEffect(() => {
    if (Platform.OS === "web") return;
    const drain = () => { void drainOfflineQueue().catch((error) => console.warn("[Kurukoo offline queue] drain failed", error)); };
    drain();
    const timer = setInterval(drain, 10_000);
    const sub = AppState.addEventListener("change", (state) => { if (state === "active") drain(); });
...
export default function RootLayout() {
  ...
  const content = deviceVerificationState ? (
    <Animated.View style={{ flex: 1, opacity: contentOpacity }}><GestureHandlerRootView style={{ flex: 1 }}><trpc.Provider client={trpcClient} queryClient={queryClient}><QueryClientProvider client={queryClient}><KurukooContextProvider><NotificationObserver /><OfflineQueueObserver /><Stack screenOptions={{ headerShown: false }}><Stack.Screen name="(tabs)" /><Stack.Screen name="oauth/callback" /><Stack.Screen name="scan" /></Stack><StatusBar style="auto" /></KurukooContextProvider></QueryClientProvider></trpc.Provider></GestureHandlerRootView></Animated.View>
  ) : (
