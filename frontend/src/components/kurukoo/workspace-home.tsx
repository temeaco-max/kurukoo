...
export function WorkspaceHome() {
  ...
  const trustedPeople = useMemo(() => ["j-whitfield"].map(entityById).filter(Boolean), []);
  const visibleTopics = useMemo(() => {
    if (topicMode === "trending") return [...topics].sort((a, b) => b.followers - a.followers).slice(0, 3);
    return [...topics].sort((a, b) => (a.posts[0]?.when ?? "").localeCompare(b.posts[0]?.when ?? "")).slice(0, 3);
  }, [topicMode]);
    ...
    <div className="min-w-0 space-y-8 pb-10 md:space-y-10">
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px] lg:items-start">
        ...
        <aside className="space-y-4">
          <FieldCard className="bg-surface">
            ...
            <div className="mt-4 flex border-y border-border/70">
              {(["trending", "new"] as const).map((mode) => <button key={mode} type="button" onClick={() => setTopicMode(mode)} className={`flex-1 px-4 py-2.5 text-[10.5px] font-medium ${topicMode === mode ? "border-b-2 border-primary text-foreground" : "text-muted-foreground hover:text-foreground"}`}>{mode === "trending" ? "Trending" : "New"}</button>)}
            </div>
            <div className="divide-y divide-border/70">
              {visibleTopics.map((topic) => <Link key={topic.slug} to="/topics/$topicSlug" params={{ topicSlug: topic.slug }} className="group block px-5 py-3.5 hover:bg-background"><div className="flex items-center justify-between gap-3"><p className="truncate text-[12px] font-semibold">{topic.name}</p><ArrowRight className="size-3 shrink-0 text-muted-foreground opacity-0 transition group-hover:opacity-100" /></div><p className="mt-1 line-clamp-2 text-[10.5px] leading-4 text-muted-foreground">{topic.blurb}</p><p className="mt-1.5 text-[9.5px] text-muted-foreground">{topic.followers.toLocaleString()} following</p></Link>)}
            </div>
          ...
          <FieldCard className="bg-surface">
            ...
            <div className="mt-4 divide-y divide-border/70">
              {trustedPeople.map((person) => person ? <Link key={person.id} to="/network/$entityId" params={{ entityId: person.id }} className="flex items-center gap-3 px-5 py-3.5 hover:bg-background"><span className="grid size-9 shrink-0 place-items-center rounded-full bg-brand-tint text-[11px] font-semibold text-brand-ink">{person.name.split(" ").map((part) => part[0]).join("").slice(0, 2)}</span><span className="min-w-0 flex-1"><span className="block truncate text-[12px] font-semibold">{person.name}</span><span className="block truncate text-[10.5px] text-muted-foreground">{person.tagline}</span></span><ArrowRight className="size-3.5 text-muted-foreground" /></Link> : null)}
            </div>
            <div className="flex items-center justify-between gap-3 border-t border-border/70 px-5 py-3"><Link to="/contacts" className="text-[10.5px] font-medium hover:text-primary">Manage people</Link><Link to="/connect" className="text-[10.5px] font-medium hover:text-primary">Connect <ArrowRight className="ml-0.5 inline size-3" /></Link></div>
          </FieldCard>

          <FieldCard>
            <div className="px-5 pt-5"><p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-muted-foreground">Agent-assisted next steps</p>{needsYou.length ? <div className="mt-3 space-y-3">{needsYou.slice(0, 3).map((item) => <Link key={item.id} to="/work/$workId" params={{ workId: item.id }} className="block border-l-2 border-primary pl-3 transition-colors hover:text-primary"><p className="text-[12px] font-medium">{item.title}</p><p className="mt-0.5 text-[10.5px] leading-5 text-muted-foreground">{item.detail}</p></Link>)}</div> : <p className="mt-3 text-[11.5px] leading-5 text-muted-foreground">No decision is waiting right now. When a consequential step needs you, Kurukoo will bring it here and keep the underlying Work item available.</p>}</div>
            <div className="border-t border-border/70 px-5 py-3"><Link to="/agents" className="inline-flex items-center gap-1 text-[10.5px] font-medium hover:text-primary">View agents <ArrowRight className="size-3.5" /></Link></div>
          </FieldCard>
        </aside>
      </div>
      ...
      <FieldCard>
        <div className="flex items-end justify-between gap-4 border-b border-border/80 px-5 py-5">
          <div><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">From creators</p><h2 className="mt-1.5 text-[18px] font-semibold tracking-[-0.02em]">Useful things to watch</h2></div>
          <Link to="/videos" className="text-[10.5px] font-medium text-muted-foreground hover:text-foreground">See all videos</Link>
        </div>
        <div className="flex gap-3 overflow-x-auto px-5 py-5 [scrollbar-width:thin]">
          {videos.map((video) => {
            const creator = entityById(video.creatorId);
            return <Link key={video.id} to="/videos/$videoId" params={{ videoId: video.id }} className="group w-[250px] shrink-0 overflow-hidden rounded-xl border border-border bg-surface transition-colors hover:border-foreground/20 hover:bg-elevated sm:w-[290px]">
              <div className="relative grid aspect-video place-items-center bg-elevated"><span className="grid size-11 place-items-center rounded-full bg-background/90 text-foreground shadow-sm transition-transform group-hover:scale-105"><Play className="ml-0.5 size-4 fill-current" /></span><span className="absolute bottom-2 right-2 rounded bg-background/90 px-1.5 py-0.5 text-[9px] font-medium">{video.duration}</span></div>
              <div className="p-3.5"><p className="line-clamp-2 text-[12.5px] font-semibold leading-5">{video.title}</p><p className="mt-1.5 truncate text-[10.5px] text-muted-foreground">{creator?.name ?? "Kurukoo creator"} · {video.views} views</p><span className="mt-2 inline-flex items-center gap-1 text-[9.5px] text-muted-foreground">{video.topic.replaceAll("-", " ")} <ArrowRight className="size-3 transition-transform group-hover:translate-x-0.5" /></span></div>
            </Link>;
          })}
        </div>
      </FieldCard>
