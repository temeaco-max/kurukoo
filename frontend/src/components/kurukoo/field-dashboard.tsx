...
export function FieldDashboard() {
  ...
  return <div className="kurukoo-field min-w-0 pb-10">
    <section className="field-welcome" aria-labelledby="field-title">
      <div className="field-welcome-copy"><p className="field-kicker">{greeting(now.getHours())}, {profileName}<span className="field-welcome-weather"><span className="field-sun" aria-hidden="true">{weather?.icon ?? "☼"}</span><span>{weather ? `${weather.temperature}°C` : "Weather unavailable"} • {weather?.location ?? fallbackLocation}</span></span></p><h1 id="field-title">Your field</h1><p className="field-subtitle">Wake up. Get going.</p></div>
      <div className="field-welcome-meta"><div className="field-date"><CalendarDays className="size-4" /><span>{now.toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" })}</span><span className="field-notification"><Bell className="size-4" />{unread ? unread : ""}</span></div></div><div className="field-date"><CalendarDays className="size-4" /><span>{now.toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" })}</span><span className="field-notification"><Bell className="size-4" />{unread ? unread : ""}</span></div></div>
    ...
    <section className="field-main-grid" aria-label="Your day">
      ...
      <div className="field-side-stack"><FieldCard className="field-agent-card"><CardHeading eyebrow="With a little help" title="Agent-assisted next steps" action={<span className="field-count">2</span>} /><Link to="/work" className="field-agent-row"><span className="field-agent-icon"><Sparkles className="size-4" /></span><span><strong>Prepare for client call</strong><small>Continue this work</small></span><ArrowRight className="size-3.5" /></Link><Link to="/work" className="field-agent-row"><span className="field-agent-icon field-agent-icon-green"><CheckCircle2 className="size-4" /></span><span><strong>Grocery top-up</strong><small>Continue this work</small></span><ArrowRight className="size-3.5" /></Link><Link to="/agents" className="field-see-all">See all suggestions <ArrowRight className="size-3.5" /></Link></FieldCard><FieldCard className="field-people-card"><CardHeading title="Trusted people" action={<Link to="/connect" className="field-card-action">Add <ArrowRight className="size-3.5" /></Link>} /><div className="field-people-row" aria-label="Trusted people">{trustedPeople.map((person, index) => <Link key={person.id} to="/profile/$entityId" params={{ entityId: person.id }} className="field-person-link" aria-label={person.name}><span className={`field-person field-person-${String.fromCharCode(97 + index)}`} aria-hidden="true">{person.name.slice(0, 1)}</span><span className="sr-only">{person.name}</span></Link>)}<Link to="/connect" className="field-person-add" aria-label="Add a trusted person">+</Link><span className="field-people-names">{trustedPeople.map((person) => person.name).join(" · ") || "No trusted people added yet"}</span></div></FieldCard></div>
    </section>

    <section className="field-topics-split"><section className="field-topics-card"><CardHeading eyebrow="Your Topics" title="What is useful around you" action={<Link to="/topics" className="field-card-action">Open Topics <ArrowRight className="size-3.5" /></Link>} /><div className="field-tabs" role="tablist" aria-label="Topic view"><button type="button" role="tab" aria-selected={topicTab === "trending"} onClick={() => setTopicTab("trending")}>Trending</button><button type="button" role="tab" aria-selected={topicTab === "new"} onClick={() => setTopicTab("new")}>New</button></div><div className="field-topic-list">{visibleTopics.map((topic) => <Link key={topic.slug} to="/topics/$slug" params={{ slug: topic.slug }} className="field-topic-row"><span><strong>{topic.name}</strong><small>{topic.blurb}</small></span><span className="field-topic-meta">{topic.followers.toLocaleString()} following <ArrowRight className="size-3.5" /></span></Link>)}</div></section><FieldCard className="field-earnings-card"><CardHeading eyebrow="Earnings" title="Your earning activity" action={<Link to="/opportunities" className="field-card-action">See all <ArrowRight className="size-3.5" /></Link>} /><div className="field-metric-list"><div className="field-metric-row"><span><strong>Tasks completed</strong><small>Requests Kurukoo saw through</small></span><span className="field-metric-value">{completedCount}</span></div><div className="field-metric-row"><span><strong>In motion</strong><small>Still being coordinated</small></span><span className="field-metric-value">{active.length}</span></div><div className="field-metric-row"><span><strong>Points</strong><small>No canonical Points balance is exposed yet</small></span><span className="field-metric-muted">Not available</span></div></div><div className="field-subhead"><p className="field-eyebrow">Opportunities</p><span className="field-count">{opportunities.length}</span></div>{opportunities.length ? <div className="field-opportunity-list">{opportunities.slice(0, 3).map((item) => <Link key={item.id} to={item.ctaLink as never} className="field-opportunity-row"><span className="field-opportunity-icon"><Sparkles className="size-4" /></span><span className="grid min-w-0 flex-1 gap-[3px]"><strong>{item.title}</strong><small>{item.subtitle}</small></span><ArrowRight className="size-3.5" /></Link>)}</div> : <div className="field-empty field-empty-compact"><Sparkles className="size-5" /><p>No opportunities right now.</p><small>Relevant ways to earn or take part will appear here.</small></div>}{opportunitiesError ? <p className="field-note">Opportunities could not be refreshed right now.</p> : null}</FieldCard></section>
