...
|---|---|---|
| Conversation | `canonicalChatTurnService` + `chatConversationService` | Implemented; delegates intelligence processing to `kurukooIntelligenceRuntime`. |
| Context arbitration | `contextArbitration` | Implemented; repository-tested. |
| Kurukoo Intelligence Runtime | `kurukooIntelligenceRuntime` | Implemented as orchestration boundary with understand → reason → selectCapabilities. The `reason()` step owns model-based planning via `modelRouter.complete(task='planning')` (Poolside → hosted → local fallback), short-circuiting for deterministic turns. Produces `IntelligenceReasoning` (plan, intent, requiresEscalation, confidence, rationale) plus a structured plan via `capabilityDiscoveryService.buildIntelligenceStructuredPlan` and propagates `modelTier`/`recommendedProvider`. Delegates to canonical services (contextArbitration, intentRouter, aiCapabilityOrchestrator). `ReasoningContext` (requiresEscalation, confidence, modelTier, structuredPlan) is consumed by `buildAICapabilityOrchestration` to adjust clarification/escalation posture and proposal confidence. Wired into `canonicalChatTurnService.processCanonicalChatTurn`. |
