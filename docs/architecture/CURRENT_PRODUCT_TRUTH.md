...
| Kurukoo Intelligence Runtime | `kurukooIntelligenceRuntime` | Implemented as orchestration boundary with understand → reason → selectCapabilities. The `reason()` step owns model-based planning via `modelRouter.complete(task='planning')` (Poolside → hosted → local fallback), short-circuiting for deterministic turns. Delegates to canonical services (contextArbitration, intentRouter, aiCapabilityOrchestrator). Wired into `canonicalChatTurnService.processCanonicalChatTurn`. |
