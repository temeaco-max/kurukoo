...
This document records the implemented convergence direction for the platform recommendations built on the existing Kurukoo architecture.
...
Individuals remain conversation-first. Business integrations are dashboard/API/integration-shaped, while both surfaces share the same Goal, capability, policy, execution and evidence substrate. No second business runtime is introduced.

## Market profiles
Current country experience remains backward compatible while `KURUKOO_MARKET_PROFILES_JSON` can add or override market metadata without changing a TypeScript country union. Market data includes currency, payment rails, regulatory class, languages and channel availability.
...
## Learning loop
Low-confidence semantic routing and legacy-router fallbacks are fed into the existing reviewed unknown-intent corpus. Only reviewed candidates carry training lineage; raw traffic is never silently converted into training data.
