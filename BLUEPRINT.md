...
|---|---|---|
| **Low-risk or guarded non-secret controls** | Voice session duration, idle, concurrency, and IP-rate limits; controlled-provider-pilot activation; consent-gated communication-outbox activation; internal credit-economy activation; bounded-agent activation, cooldown, action, retry, concurrency, and worker interval limits | Editable only when the Control Plane is enabled and the deployment has not locked the control. Input is range- and type-validated. The live voice rate limit and outbox pause are evaluated inside their existing canonical boundaries; the agent worker interval is persisted for the next managed worker start. |
...
The shared Control Room shell provides one responsive sidebar, fixed operational header, compact Signals drawer, and a uniform **Kurukoo operating system / Control room** heading treatment. Its grouped navigation exposes every existing authoritative operator workspace: control room, operations, control plane, pilot observability, users and providers, supply registry, AI agents, skill flows, safety reports, CMS, SEO, social scheduling, marketing, artist and celebrity programs, pricing, revenue, commissions, referrals, partnerships, analytics, and roadmap. Desktop navigation may collapse to a compact rail, but the fixed header must retain a visible, labelled control that restores the full navigation; narrow viewports use the existing menu control. It does not create alternate data stores or unauthorised bulk mutations.

The Control Room dashboard is mandate-led: it states the conversation-first, evidence-before-execution boundary before presenting operating signals. Its browser client reads one authenticated `/api/admin/dashboard` snapshot that composes the existing stats, commercial metrics, skill-flow, provider-presence, CMS, and SEO services. This prevents dashboard rendering from multiplying rate-limited calls or adding another data store. Every region resolves to a recorded value, a zero/empty recorded state, or an explicit retryable unavailable state; a missing SEO audit is **Not recorded**, not `0/100`. Dashboard values remain descriptive records only and must not imply provider selection, message delivery, payment settlement, publication, crawler results, or external execution.

The Control Plane also provides an **integrated-system registry**. It shows secret-safe readiness and a link to the owning surface for live voice, the bounded agent runtime, the consent-gated communications outbox, the internal credit economy, the deployment-owned payment adapter, USSD readiness, and background workers. A registry state means only that the relevant configuration boundary is present or disabled; it never represents payment collection, provider verification, delivery, external publication, fulfilment, or other real-world completion.
...
The SEO Management Console uses the existing authenticated `/api/admin/seo/*` boundary for metadata, redirects, keywords, backlinks, calendar items, content briefs, internal links, FAQs, schema templates, 404 records, audits, rankings, orphan discovery, image metadata, and health checks. Every control maps to a concrete canonical action: add and keyed upsert records save through the documented POST boundary; mutable record edits use their PUT or keyed-upsert path; delete requires an explicit confirmation and uses the matching DELETE boundary; refresh reloads that resource group; and audit/generation actions show a busy state and refresh their persisted result. A content brief may hand off into a prefilled CMS draft, and CMS **Save & review SEO** persists the content record before opening the matching public-path metadata record. AI-assisted SEO outputs remain drafts subject to the existing human-review, quota, and truthfulness requirements; they do not invent rankings, backlinks, public availability, or external Search Console evidence.

The public `/resources` and homepage guide surfaces project only the existing CMS `help`, `page`, and `legal` records through `/api/resources`; chat may surface the same published guide links as supplementary reading. Public resources are rendered through the shared SEO head path for their matching route. Neither a CMS save, a resource card, a metadata record, an audit score, nor an empty SEO dashboard constitutes a crawler result, a rank, a live service, a provider-availability statement, an external delivery, or fulfilment evidence. The SEO dashboard reports actual saved record counts and the last recorded audit state only; rankings remain unavailable until a verified ingestion path writes them.
...
| SEO, Discover, Daily Picks, opportunities, and advertising | Existing SEO, discovery, Daily Picks, opportunity, and advertising owners | SEO/Discover and a quality-gated Topic Daily Picks projection are implemented through their existing owners. Sponsorship, provider/business links, and any other projection still require their own truth, quality, privacy, disclosure, and regression gate. |
...
---
### PWA Offline Cache-Boundary Addendum (2026-08-13)

The existing service worker now versions and migrates its caches to retain only explicit public shell assets and a closed allow-list of public marketing/help pages. No dynamic API response is eligible for persistence. It never persists unknown navigations, authenticated workspaces, admin routes, chat or attachment routes, voice sessions/transcripts, account/profile/points/reminder/safety/task data, orders, Pulse data, or webhooks. An uncached navigation may display only the existing offline fallback when the network is unavailable; it is never added to a cache. The PWA does not promise offline chat, request, payment, account, private media, or dynamic provider data.

The PWA regression verifies versioned cache migration, explicit allow-list routing, private dynamic exclusions, same-origin public-cache enforcement, service-worker registration, and truthful offline copy.
...
The server validates the declared data-URL content type, strict Base64 form, per-type size limit, and a matching binary signature. It chooses the on-disk extension from the validated type rather than the supplied filename. Retrieval and deletion require the same authenticated owner; downloads use an attachment disposition, explicit content type, `nosniff`, and `private, no-store`. The chat stream replaces any client-supplied attachment metadata with the owner-scoped stored record before it persists the existing message, and rejects guest or cross-owner attachment references. The established protected `deleteUserData` lifecycle calls the chat attachment owner before removing the profile, so account deletion removes the owner’s attachment metadata and private bytes or fails intact for a retry.

The canonical conversation deletion owner also extracts attachment references from its own message metadata. It deletes private bytes only after the owner’s final remaining chat reference is removed, preserving shared references until then. Both active scheduled 12-month retention paths use the same bounded canonical message cleanup (up to 1,000 expired messages per run), so expired messages cannot leave metadata or final attachment bytes behind. Conversation clearing first verifies ownership before it can remove message metadata, messages, or attachments, so a cross-owner conversation identifier cannot affect another user’s chat state. The protected user-data export now composes a bounded (maximum 250 records each) owner-scoped chat-message snapshot and active attachment metadata from these same owners, with explicit scope flags that attachment bytes and storage paths are excluded. The protected account-deletion service removes all owner-scoped chat history through the conversation owner before it removes residual private uploads and the profile. It also invokes the existing reminder, safety, deferred-intention, and opportunity owners to remove private workspace state (reminders, safety contacts/check-ins, unfinished intentions, and Daily Picks records). Economic, payment, escrow, dispute, verification, trust, provider-supply, and compliance/audit records are not casually deleted by this path. For the existing canonical Economic Request lifecycle, terminal customer or provider participation is identity-anonymised (`phone`, `provider_phone`, linked participant/offer identity, and customer requirements) while request status, skill/category, capabilities, quote/fulfilment, payment, escrow, dispute, and audit integrity remain. Any active Economic Request causes account deletion to fail closed before any other private-state cleanup, returning its owner a conflict to resolve or cancel the request first. Other retention and identity-anonymisation boundaries remain explicit rather than being falsely represented as erased. The attachment regression covers unauthenticated upload rejection, malformed content rejection, owner-only bounded export, private non-static storage, owner-only retrieval/deletion, message/conversation/account-deletion cleanup, forced-download headers, cross-user rejection, and stream-level metadata ownership. Virus scanning, content moderation of binaries, object storage, encrypted-at-rest key management, durable retention/cleanup policy, and multi-instance media storage remain deployment requirements; no code path represents the current pilot file store as permanent or publicly shareable media.

---

### HTTP Browser-Security Delivery Addendum (2026-08-13)
The Express composition root is the single HTTP browser-security boundary for every public page, static asset, administrative HTML page, API response, protected attachment response, and route fallback. It removes framework disclosure and consistently emits `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`, and a deliberately narrow permissions policy (`geolocation` and `microphone` only for the same origin; camera and Payment Request disabled). Production responses additionally emit one-year HSTS with subdomains.

This global floor complements rather than replaces route-specific controls: attachment downloads retain forced-download, `private, no-store`, trusted content type, and owner authorization; dynamic presence and other private boundaries preserve their own cache semantics. The policy deliberately does not add a speculative Content Security Policy before the complete template and third-party asset contract has been independently verified. The real-app HTTP-header regression covers public, static, administrative, dynamic API, voice, and authenticated attachment boundaries and checks that their established privacy cache requirements remain intact.
---

### FCM Notification Transport Addendum (2026-08-13)
FCM is an optional outbound adapter beneath the existing notification, consent, communication-delivery, and outbox owners; it is not a second notification authority. An authenticated existing profile may register or remove only its own bounded device token through the existing notification boundary. Tokens are not returned by the API, logged, or copied into durable outbox payloads. The dispatch adapter resolves the current owner-bound token privately at send time and uses a short-lived OAuth token derived from `FCM_SERVICE_ACCOUNT_JSON` only when a valid service-account JSON secret is configured.

Every push first creates the existing durable in-app notification fallback. External FCM dispatch requires explicit `fcm` communication consent, the shared outbox lease/retry path, and the shared communication-delivery ledger. Provider HTTP v1 acceptance records an opaque provider reference and `accepted` state only. It does not establish per-device delivery, display, open, or read: FCM offers no synchronous per-message receipt callback for this boundary. An unconfigured adapter becomes `not_configured`, a rejected unregistered token is cleared only if it still belongs to that same owner, and the in-app fallback remains available. Aggregate provider analytics or BigQuery exports, when deliberately enabled with user consent, are operational reporting inputs rather than individual delivery receipts.
