...
The existing service worker now versions and migrates its caches to retain only the explicit public shell assets, a closed allow-list of public marketing/help pages, and the three public metadata APIs needed by that shell. It never persists unknown navigations, authenticated workspaces, admin routes, chat or attachment routes, voice sessions/transcripts, account/profile/points/reminder/safety/task data, orders, Pulse data, or webhooks. An uncached navigation may display only the existing offline fallback when the network is unavailable; it is never added to a cache. The PWA does not promise offline chat, request, payment, account, private media, or dynamic provider data.
