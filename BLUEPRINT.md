...
The attachment regression covers unauthenticated upload rejection, malformed content rejection, private non-static storage, owner-only retrieval/deletion, account-deletion cleanup, forced-download headers, cross-user rejection, and stream-level metadata ownership. Virus scanning, content moderation of binaries, object storage, encrypted-at-rest key management, durable retention/cleanup policy, and multi-instance media storage remain deployment requirements; no code path represents the current pilot file store as permanent or publicly shareable media.
