...
## Current Convergence Addendum — Canonical AI Routing, Provider Resilience, and Controlled Learning

Kurukoo’s canonical skill catalogue is now machine-derived through `skillCatalogueConvergence.ts`: **241 skills (205 core and 36 market extensions)**. Runtime skill resolution, execution contracts, behaviour-pack compilation, FastText corpus generation, SmolLM2 training-universe generation, scenario generation, and outcome audits consume that one catalogue. The prior extension-category review found 17 mismatches; each is now either corrected or represented as a documented deliberate override, with permanent catalogue and outcome regressions preventing an undocumented mismatch from re-entering the system.

FastText remains a **cheap, non-authoritative routing signal**. Deterministic conversation acts such as greeting, thanks, confirmation, rejection, correction, clarification, status, cancellation, how-to, and ordinary conversation are resolved before domain selection. For requests requiring routing, the classifier produces an abstention-capable category signal and bounded in-category skill candidates; semantic ambiguity, close candidates, and multi-step wording can then use the existing local model boundary. A model does not authorize payment, safety, identity, provider state, execution, completion, or evidence.

| Control | Canonical behavior | Truthfulness boundary |
| --- | --- | --- |
| FastText configuration | Thresholds, class caps, cache limits, and feedback retention are owned by `fastTextRoutingConfig.ts`. | The executable is unavailable in the local validation host, so deterministic fallback attribution is retained and no model-accuracy claim is made. |
| Corpus and evaluation | Curated UK, Nigerian English, Pidgin, market-language, Canadian, incomplete, typo, and hard-negative inputs are merged with capped bootstrap data. The current reproducible split is 778 train, 57 validation, and 57 held-out test examples across 312 labels. | The committed evaluation manifest records configuration and boundary outcomes; it does not claim production traffic accuracy. |
| Unknown inputs | Uncertain inputs enter a privacy-redacted, deduplicated review queue with reviewer decision and training lineage. | Raw traffic is never auto-trained; an accepted review produces lineage only until a deliberate corpus and evaluation cycle occurs. |
| Unified routing | `unifiedAiEngine` records an explicit escalation reason, starts from canonical routing, uses local inference before justified hosted reasoning, and never grants model mutation authority. | Provider selection is not provider verification or evidence of external execution. |
| Cache safety | The shared dependency-free LRU refreshes recency on read and excludes prompts that depend on owner-private memory. | No cached response may cross a user boundary or become a source of mutable commercial truth. |

Hosted AI providers use one shared circuit state machine for Mistral, Gemini, Groq, and OpenRouter: `healthy`, `degraded`, `open`, and `half_open`. It captures consecutive failures, timeouts, rolling error rate, timestamps, cooldown, and a single recovery probe. Open providers are skipped before an upstream wait; degraded providers are de-preferred. Existing quota accounting records request-level provider/model/cost-status, latency, success, fallback, agent, skill, category, country, and escalation-reason telemetry. Unavailable pricing remains `estimated_cost = null` and `cost_status = unavailable`; it is never estimated from an invented rate.

First-class AI agents retain their identity, skills, tool boundaries, administrator controls, daily quota, concurrency limits, and safety checks. The bounded runtime now adds per-goal tokens, per-cycle tokens, daily hosted-escalation limits, and a cooldown after exhaustion. When a hosted escalation limit is reached, the task is constrained to the existing local inference boundary; an exhausted goal waits rather than consuming retries as a cost-control substitute. The protected admin API exposes AI telemetry, all shared provider health snapshots, and per-agent budget status. The coordinator’s request recheck is intentionally deterministic: it invokes the canonical owned-request/storefront tool and performs **no LLM call**; therefore it already satisfies the strongest cost gate of no-model recheck before any SmolLM2 or hosted escalation path.

The canonical behaviour compiler derives all 241 skill entries from taxonomy, `skillExecutionContract`, existing skill flows, universal capabilities, and the agent-tool registry. It does not construct 241 bespoke engines. Its latest compiled validation covers 241 skills, 262 capabilities, nine agent tools, fourteen behaviour families, and the existing scenario lab verifies 24,000 trajectories across 241 skills, 46 families, seven market configurations, ten channels, and twenty lifecycle variants.

## Current Convergence Addendum — Discovery Provenance, Credential Control, and Persistence Safety

The existing `discoveryNetwork` remains the sole discovery owner. Discovery entities now carry explicit source provenance: `verified_provider`, `business_listing`, `authoritative_directory`, `licensed_external`, or `opportunity_signal`. The service preserves source attribution and evidence level without asserting availability. Its query contract also returns an explicit density readiness state: `ready`, `sparse`, or `no_verified_local_data`. The latter returns the neutral recovery statement, “Not much is live here yet. Ask Kurukoo what you need and we’ll help find it.” Public discovery routes carry safe source type and readiness metadata without exposing exact coordinates or source references.

Provider credential controls are an authenticated backend surface, not a raw environment editor. Approved credential families include Gemini, Groq, Mistral, OpenRouter, Stripe, WhatsApp, Telegram, Resend, and Firebase. Secrets are encrypted at rest using the existing protected encryption primitive and are never returned in plaintext. The control surface provides masked metadata, rotation version and timestamp, disable, revoke, actor-scoped audit, last test state, and connection testing where a provider-specific safe probe is available. A test result reports only credential acceptance or failure; it never activates a provider integration or claims live production traffic.

High-write persistence has one active owner today: single-worker SQL.js. The shared high-write persistence boundary covers messages/conversations, durable jobs, and the commercial ledger, rejects multi-worker writes under that active mode, and reports a Postgres connection signal only as **configured but unactivated**. It enforces a single-owner/no-dual-write migration policy. No Postgres adapter, multi-worker deployment, or cross-database ownership is claimed until a tested adapter migration, validation, and explicit cutover are completed. The existing scale-transition readiness report remains the operator-facing deployment safeguard.

The route, FastText, economic lifecycle, discovery, credential, high-write persistence, scenario, AI-resilience, and TypeScript regressions provide repository-side evidence for these controls. They do not demonstrate external provider activation, hosted-model availability, real credential acceptance, payment settlement, delivery, production traffic accuracy, or multi-instance persistence safety.
