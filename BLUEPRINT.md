...
The canonical conversation deletion owner also extracts attachment references from its own message metadata. It deletes private bytes only after the owner’s final remaining chat reference is removed, preserving shared references until then. Conversation clearing first verifies ownership before it can remove message metadata, messages, or attachments, so a cross-owner conversation identifier cannot affect another user’s chat state. The attachment regression covers unauthenticated upload rejection, malformed content rejection, private non-static storage, owner-only retrieval/deletion, message/conversation/account-deletion cleanup, forced-download headers, cross-user rejection, and stream-level metadata ownership. Virus scanning, content moderation of binaries, object storage, encrypted-at-rest key management, durable retention/cleanup policy, and multi-instance media storage remain deployment requirements; no code path represents the current pilot file store as permanent or publicly shareable media.
