...
The canonical `ad_campaigns`/`adManager` authority now owns the configured placement catalogue, embedded campaign creative, campaign-to-slot assignment, active/paused/completed eligibility, canonical category and coarse active-market targeting, frequency caps, bounded delivery-unit budgets, disclosure, event-token-protected aggregate impression/click evidence, campaign CTR, and provider-neutral programmatic readiness status. It reuses the existing affiliate owner for the only approved public fallback: **direct campaign → approved matching affiliate offer → empty**. No campaign resolves from a raw chat query, Memory Profile, private request text, precise location, or inferred sensitive attribute.

The same authenticated Admin Marketing Control Room lists campaign metrics, approved placement occupancy, activation and frequency controls, placement assignment, category/market targeting, and the explicit unconfigured-programmatic boundary. Responsive lazy hosts are shared across homepage, How It Works, Explore/category, Discover feed, Topic list/detail, Resources/articles, provider-related content, and the existing workspace sponsorship/affiliate surfaces. They render no card when inventory is absent. Chat message streams and all auth, security, private communication, safety, sensitive, payment/escrow/dispute, voice, attachment, verification, moderation, administrative, and inactive-market contexts are excluded.

Every served card remains clearly labelled and explicitly separated from providers, assistant messages, community content, editorial guidance, request state, stock, price, availability, quote, booking, payment, delivery, and fulfilment. Event counts are local deduplicated evidence—not third-party viewability, conversion, billed spend, advertising revenue, settlement, or external delivery evidence. Programmatic advertising remains disabled until a reviewed adapter, credentials, consent/privacy assessment, approved format policy, and acceptance/error contract are separately supplied.
...
Daily Picks keeps personal context and sponsorship separate. Its dedicated sponsor panel resolves transiently through `adManager`’s `workspace_daily_picks_sponsor` placement after receiving only canonical deferred-request category context; it never sends raw request text, chat, Memory, or personal-profile data into advertising selection. A pause, delivery-unit cap, placement deactivation, or eligibility failure removes sponsorship immediately, and sponsorship never appears among personal suggestions. The generic static promotion preview and unbacked proactive question remain removed. The opportunity and advertising regressions cover active eligibility, disclosure, owner isolation, category-only context, and immediate pause exclusion.
...
Attachments have no public static URL. Download and deletion require the same owner and send attachment disposition, explicit type, `nosniff`, and `private, no-store`; a chat stream resolves an attachment id to the canonical owner-scoped record before persisting message metadata. The protected account-data export now composes only a bounded owner-scoped chat-message snapshot and active attachment metadata, explicitly excluding bytes and storage paths. Account deletion uses the canonical conversation owner first to remove all owner-scoped chat messages and their final attachment references, then the attachment owner removes residual private uploads before profile removal or preserves the account intact if file cleanup fails. Both active scheduled 12-month retention paths invoke the same bounded canonical cleanup (up to 1,000 expired messages per run), so expired messages cannot leave chat metadata or final attachment bytes. The protected account-deletion owner also calls the existing reminder, safety, deferred-intention, and opportunity owners to remove private workspace state. Economic/payment/escrow/dispute, verification, trust, provider-supply, and compliance/audit records remain intentionally retained or identity-anonymised by their own established boundaries. The canonical Economic Request owner now identity-anonymises terminal customer/provider participation and customer requirements while preserving the request lifecycle, evidence, quote/fulfilment, payment, escrow, dispute, and audit records; an active request blocks account deletion with a truthful conflict before any other personal-state cleanup. The account flow does not claim to erase governed evidence records. The canonical message/conversation deletion owner removes attachment bytes only after the final owner-scoped message reference is gone, and verifies conversation ownership before any conversation metadata is removed. The new real-app regression covers unauthenticated, malformed, cross-owner, guest-stream, retrieval, direct deletion, bounded export, message/conversation/account deletion, shared-reference retention, and static-hosting boundaries. Binary malware scanning, full content moderation, object storage, durable cleanup/retention, encrypted-at-rest key management, and multi-instance media durability remain deployment work rather than claims of the current SQL.js pilot.
