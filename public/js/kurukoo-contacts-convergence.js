(() => {
  ...
  const communicationAction = (person) => {
    ...
    const unavailable = !person.communication?.call ? '<span class="k57-contact-meta">Calling is not available right now.</span>' : '';
  ...
  const render = (contacts) => {
    ...
    section.innerHTML = `<div class="k-app-card"><span class="k-app-card-label">People you know</span><h2 id="k57-contacts-title">People connected to your Kurukoo</h2><p>Keep the people who matter to your work in one place. Messaging and calling are only shown when the related service is ready.</p><form class="k57-contact-add" data-k57-add><label>Contact phone or identity<input name="personPhone" autocomplete="tel" inputmode="tel" required placeholder="Enter a known Kurukoo contact"></label><button class="k-app-primary" type="submit">Add person</button></form><div class="k57-contact-state" data-k57-add-state></div></div><section class="k-app-card"><div class="k57-contact-grid" data-k57-list></div><div class="k57-unavailable" data-k57-empty hidden><div><h3>No people here yet</h3><p>Add a known Kurukoo contact when you want to keep that relationship close to your work.</p></div></div></section></section>`;
    ...
    contacts.forEach(async (person) => {
      ...
      card.innerHTML = `<div class="k57-contact-head"><div class="k57-avatar" aria-hidden="true">${esc(avatar.initials)}</div><div><h3>${esc(person.displayName)}</h3><div class="k57-contact-meta"><span>${esc(String(person.presence || 'Presence unavailable'))}</span>${person.provider ? `<span>${person.provider.verified ? 'Verified provider' : 'Provider'}${person.provider.available ? ' · available' : ''}</span>` : ''}</div></div></div><div class="k57-contact-meta"><span>Connection: ${esc(person.relationship?.status || 'not set')}</span>${person.relationship?.safetyContact ? '<span>Safety contact</span>' : ''}</div><div class="k57-contact-actions">${communicationAction(person)}<button class="workspace-text-action" type="button" data-k57-follow>${activeFollow ? 'Stop following' : 'Follow updates'}</button><button class="workspace-text-action" type="button" data-k57-remove>Remove</button></div>`;
