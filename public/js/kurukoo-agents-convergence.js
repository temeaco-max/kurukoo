(() => {
  ...
  const goalPresenceLabel = (status) => ({ active: 'Working now', working: 'Working now', waiting: 'Waiting for an update', waiting_on_dependency: 'Waiting for earlier work', needs_user: 'Needs your approval', blocked: 'Kurukoo needs something to continue', completed: 'Done', cancelled: 'Stopped', failed: "Couldn't complete this yet", expired: 'No longer active', unavailable: 'Unavailable' }[String(status || '').toLowerCase()] || 'Updating');
  const activityLabel = (event) => {
    const kind = String(event?.kind || '');
    const status = String(event?.status || '');
    if (kind === 'goal_started') return 'Kurukoo started moving this forward.';
    ...
    if (kind === 'outcome') return status === 'completed' ? 'Kurukoo verified the outcome.' : `Kurukoo updated this work: ${humanize(status)}.`;
    ...
    return 'Kurukoo updated this work.';
  };

  const safeNextAction = (nextAction) => {
    const value = String(nextAction || '').trim().slice(0, 160);
    if (!value || /(?:goal:|[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}|token|secret|prompt|argument|payload|canonical|capability|runtime|executor|worker|trace)/i.test(value)) return 'Kurukoo will continue this work when it is ready.';
    return value;
  };
  ...
  const render = (goals, continuations = new Map(), traces = new Map()) => {
    ...
    list.innerHTML = goals.slice(0, 25).map((goal) => {
      ...
      const objective = esc(String(goal.objective || goal.summary || 'Work in progress'));
      ...
      const href = `/chat?prompt=${encodeURIComponent('Continue this work.')}`;
      return `<article class="k-agent-goal-row">
        ...
        <a class="k-agent-goal-link" href="${href}">${ready ? 'Continue with Kurukoo' : 'See what is needed'} →</a>
      </article>`;
    }).join('');
  };
  ...
  const load = async () => {
    try {
      ...
      if (empty) { empty.hidden = false; const title = empty.querySelector('h3'); if (title) title.textContent = 'Work overview is unavailable'; }
      list.setAttribute('aria-busy', 'false');
      list.innerHTML = '';
    }
  };
