...
|---|---|---|
| **P0** | Configure a real PSP, provider/identity verification, and push delivery/receipt adapter. | Production claims are supported by credentialed external evidence, expiry/revocation handling, consent, and observable delivery/settlement results. |
| **P0** | Owner-side credential rotation and protected-branch controls. | Historical credential exposure is remediated outside source code; `main` requires reviewed passing checks. |
